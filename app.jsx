      ...
      <div onClick={onAccept} style={{
        ...
        <div style={{padding:'14px 16px 12px', display:'flex', alignItems:'flex-start', gap:12}}>
          <div style={{width:44, height:44, borderRadius:'50%', flexShrink:0,
            background:'rgba(255,255,255,0.15)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22}}>
            💧
          </div>
          <div style={{flex:1, minWidth:0}}>
            <div style={{fontSize:10.5, fontWeight:800, color:'rgba(255,255,255,0.65)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:2}}>
              {lang==='pt'?'Nova vaga disponível':lang==='es'?'Nuevo trabajo disponible':'New job available'}
            </div>
            <div style={{fontSize:15, fontWeight:800, color:'#fff', lineHeight:1.25}}>{alert.title}</div>
        ...
        <div style={{display:'flex', gap:0, borderTop:'1px solid rgba(255,255,255,0.15)'}}>
          <button onClick={(e)=>{ e.stopPropagation(); onDismiss(); }} style={{
            ...
            background:'transparent', color:'rgba(255,255,255,0.75)', fontSize:13, fontWeight:700,
            borderRight:'1px solid rgba(255,255,255,0.15)',
          }}>
            {lang==='pt'?'Ignorar':lang==='es'?'Ignorar':'Ignore'}
          </button>
          <button onClick={onAccept} style={{
            flex:1.4, padding:'12px', border:'none', cursor:'pointer', fontFamily:'inherit',
        ...
        <div style={{height:3, background:'rgba(255,255,255,0.15)'}}>
          <div style={{height:'100%', background:'#fff', animation:'pg-ride-drain 18s linear forwards'}}/>
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  React.useEffect(() => {
    ...
    const qpChannel = window.sb.channel('quickpool-broadcast')
      .on('broadcast', { event: 'new_job' }, ({ payload: job }) => {
        ...
        window.playNotifSound && window.playNotifSound();
        if (navigator.vibrate) try { navigator.vibrate([120, 60, 120]); } catch(e) {}
        setRideAlert({ title, body, url: `/#quick?job=${job.id}` });
      })
      .subscribe();
            ...
            try {
              ...
              if (!scheduledFor) {
                ...
                qpBroadcastRef.current?.send({
                  type: 'broadcast', event: 'new_job',
                  payload: { id: inserted.id, city: inserted.city, day_of_week: inserted.day_of_week,
                    poster_id: inserted.poster_id, pools_count: inserted.pools_count, price_per_pool: inserted.price_per_pool },
                }).catch(()=>{});
                try {
