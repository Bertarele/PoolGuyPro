...
function App() {
  ...
  const loadProfile = React.useCallback(async (sbUser) => {
    ...
    if (profile?.tier && profile.tier !== 'free') {
      window.sb.from('subscription_events').select('billing')
        .eq('user_id', sbUser.id).order('created_at', { ascending: false }).limit(1)
        .then(({ data }) => {
          const billing = data?.[0]?.billing;
          if (billing) setUser(u => ({ ...u, tierBilling: billing }));
        })
        .catch(() => {});
    }
  ...
  const loadPendingRatings = React.useCallback(async () => {
    ...
    try {
      ...
      const { data: received } = await window.sb.rpc('my_incoming_ratings');
  ...
  React.useEffect(() => {
    ...
    const check = async () => {
      if (!window.sb || !user?.uid) return;
      const { data: received } = await window.sb.rpc('my_incoming_ratings').catch(() => ({ data: null }));
      if (!received) return;
  ...
  const manageSubscription = async (action) => {
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        showToast('❌ ' + (data?.error === 'no_subscription'
          ? (lang==='pt' ? 'Nenhuma assinatura ativa encontrada.' : lang==='es' ? 'No se encontró ninguna suscripción activa.' : 'No active subscription found.')
          : (data?.message || 'Error')));
        return;
      }
      setUser(u => ({ ...u, tierCancelAt: data.cancel_at || null }));
      showToast(action === 'cancel'
        ? (lang==='pt' ? '✓ Assinatura cancelada — seu acesso continua até o fim do período pago.' : lang==='es' ? '✓ Suscripción cancelada — tu acceso sigue hasta el fin del período pagado.' : '✓ Subscription canceled — your access continues until the paid period ends.')
        : (lang==='pt' ? '✓ Assinatura reativada — vai renovar normalmente.' : lang==='es' ? '✓ Suscripción reactivada — se renovará normalmente.' : '✓ Subscription resumed — it will renew normally.'));
    } catch (e) {
  ...
  React.useEffect(() => {
    ...
    window.sb.rpc('claim_referral', { p_code: code }).then(({ data, error }) => {
      ...
      if (data.ok) {
        ...
        showToast(lang==='pt'
          ? `🎁 Indicação aplicada! Seu desconto será aplicado na assinatura.`
          : lang==='es'
          ? `🎁 ¡Referido aplicado! Tu descuento se aplicará en la suscripción.`
          : `🎁 Referral applied! Your discount will apply at checkout.`);
        loadWallet();
      }
    }).catch(() => {});
  ...
  const ctx = {
    ...
    manageSubscription, subManageBusy,
  ...
  const OverlayBundle = () => (
    <>
      ...
      {showOnboarding && (() => {
        const slides = {
          en: [
            { icon:'🏊', title:'Welcome to PoolGuyX', desc:'The marketplace built for Florida pool professionals. Find jobs, post routes, and connect with other pool guys.' },
          ...
          pt: [
            { icon:'🏊', title:'Bem-vindo ao PoolGuyX', desc:'O marketplace feito para profissionais de piscina na Flórida. Encontre trabalhos, publique rotas e conecte-se com outros pool guys.' },
          ...
          es: [
            { icon:'🏊', title:'Bienvenido a PoolGuyX', desc:'El marketplace para profesionales de piscinas en Florida. Encuentra trabajos, publica rutas y conéctate con otros pool guys.' },
          ...
          <nav style={{
            ...
            <div style={{padding:'0 10px 12px', display:'flex', flexDirection:'column', gap:1}}>
              ...
              <div style={{padding:'8px 14px 2px', display:'flex', alignItems:'center', gap:6}}>
                ...
                <span style={{fontSize:9, color:'rgba(255,255,255,0.13)', letterSpacing:'0.06em', whiteSpace:'nowrap'}}>{APP_VERSION}</span>
                <div style={{flex:1, height:1, background:'rgba(255,255,255,0.04)'}}/>
              </div>
            </div>
          </nav>
