...
function RideRequestCard({
  ...
  }, alert.body)), hasStructured && /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: 'right',
      flexShrink: 0
    }
  ...
  }, infoChips.map((chip, i) => /*#__PURE__*/React.createElement("span", {
    key: i,
    style: {
      fontSize: 11.5,
      fontWeight: 700,
  ...
  }, lang === 'pt' ? 'Ver anúncio' : lang === 'es' ? 'Ver anuncio' : 'View listing'), /*#__PURE__*/React.createElement("button", {
    onClick: onApply,
    disabled: applying,
    style: {
      flex: 1.3,
  ...
  }, applying ? lang === 'pt' ? 'Enviando…' : lang === 'es' ? 'Enviando…' : 'Sending…' : lang === 'pt' ? 'Candidatar-se' : lang === 'es' ? 'Postularme' : 'Apply')), /*#__PURE__*/React.createElement("div", {
    style: {
      height: 3,
...
function App() {
  ...
  React.useEffect(() => {
    if (!window.sb) return;
    window.sb.from('app_config').select('plans_enabled').eq('id', true).single().then(({
      data
    }) => {
      if (data) setPlansEnabled(data.plans_enabled !== false);
    }).catch(() => {});
  }, []);
  ...
  const loadProfile = React.useCallback(async sbUser => {
    ...
    try {
      setTweak('tier', profile?.tier || 'free');
    } catch (e) {}
  ...
  const handleAuthLogin = React.useCallback(async sbUser => {
    ...
    try {
      const ll = localStorage.getItem('pg_lang');
      if (sbUser && sbUser.user_metadata && ll && sbUser.user_metadata.lang !== ll) window.sb.auth.updateUser({
        data: {
          lang: ll
        }
      });
  ...
  const [pendingDeepLink, setPendingDeepLink] = React.useState(() => {
    try {
      ...
      if (hash.startsWith('#chat')) {
        ...
        const qs = hash.includes('?') ? hash.slice(hash.indexOf('?') + 1) : '';
        window.history.replaceState(null, '', '#home');
        if (new URLSearchParams(qs).get('rate') === '1') return {
          type: 'rate'
        };
      }
    } catch {}
  ...
  const handleRideApply = React.useCallback(async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('pgQuickPoolApplied', {
        detail: {
          jobId
        }
      }));
  ...
  React.useEffect(() => {
    ...
    const doFetch = async () => {
      const [j, tc, v, m, ho, mqp] = await Promise.all([window.sb.from('jobs').select('*').order('created_at', {
        ascending: false
      ...
      user?.uid ? window.sb.from('quick_pool_jobs').select('*').eq('poster_id', user.uid).in('status', ['open', 'filled']).order('created_at', {
        ascending: false
      }) : Promise.resolve({
        data: []
      })]);
  ...
  React.useEffect(() => {
    if (!authReady || !user?.uid || !window.sb) return;
    const check = () => window.sb.from('notifications').select('id').eq('user_id', user.uid).eq('read', false).then(({
      data
    }) => {
  ...
  React.useEffect(() => {
    setUser(u => ({
      ...u,
      tier: plansEnabled ? t.tier : 'premium',
      realTier: t.tier,
      betaFreeForAll: !plansEnabled
    }));
    try {
  ...
  const OverlayBundle = () => /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(ChatSheet, {
    ...
    wallet: wallet,
    showToast: showToast,
    betaFreeForAll: user.betaFreeForAll
  }), showOnboarding && (() => {
    const slides = {
      en: [{
        icon: '🏊',
    ...
    onSubmit: data => {
      ...
      const overlap = liveVacations.some(v => v.author_id === user.uid && (!editingVac || v._id !== editingVac._id) && v.monthIdx === data.monthIdx && v.year === data.year && (v.selectedDays || []).some(d => (data.selectedDays || []).includes(d)));
      if (overlap) {
        setEditingVac(null);
    ...
    myAppliedDays: dayPickerVac ? liveApplications.filter(a => String(a.job_id) === String(dayPickerVac._id) && a.status === 'pending').flatMap(a => a.vacation_days?.selectedDays || []) : [],
    onClose: () => setDayPickerVac(null),
    onSubmit: async data => {
      ...
      if (error) {
        showToast('❌ ' + (lang === 'pt' ? 'Erro ao enviar candidatura' : 'Failed to submit application'));
        return;
      }
      ...
      if (ownerId && ownerId !== user.uid) {
        ...
        window.sendPush && window.sendPush(ownerId, lang === 'pt' ? '📬 Nova candidatura' : lang === 'es' ? '📬 Nueva postulación' : '📬 New application', lang === 'pt' ? `${user.name || 'Alguém'} se candidatou para cobrir suas férias.` : lang === 'es' ? `${user.name || 'Alguien'} se postuló para cubrir tus vacaciones.` : `${user.name || 'Someone'} applied to cover your vacation.`, '/#work', 'work');
      }
      showToast(lang === 'pt' ? '✓ Candidatura enviada' : lang === 'es' ? '✓ Postulación enviada' : '✓ Application sent');
      loadLiveApplications(user.uid);
    }
