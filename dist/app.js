...
function App() {
  ...
  React.useEffect(() => {
    ...
    const ch = window.sb.channel('ratings-live-' + user.uid).on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'ratings',
      filter: `to_id=eq.${user.uid}`
    }, p => {
  ...
  React.useEffect(() => {
    ...
    const channel = window.sb.channel('app-realtime').on('postgres_changes', {
      ...
      schema: 'public',
      table: 'techs'
    }, p => setLiveTechs(prev => prev.map(tc => tc._id === p.new.id ? normTech(p.new) : tc))).on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'vacations'
