...
function ExpiringListingPrompt({
  item,
  lang,
  onClose,
  onRenew,
  ...
  }, "\u23F3 ", lang === 'pt' ? 'Seu anúncio expira em breve' : lang === 'es' ? 'Tu anuncio expira pronto' : 'Your listing expires soon'), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: 'var(--pg-ink-500)',
      marginBottom: 20,
  ...
  }, lang === 'pt' ? `"${item.name}" ainda está disponível?` : lang === 'es' ? `¿"${item.name}" sigue disponible?` : `Is "${item.name}" still available?`), /*#__PURE__*/React.createElement("button", {
  ...
  }, "\u2713 ", lang === 'pt' ? 'Sim, renovar por 30 dias' : lang === 'es' ? 'Sí, renovar por 30 días' : 'Yes, renew for 30 more days'), /*#__PURE__*/React.createElement("button", {
...
function MarkSoldSheet({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    points: "20 6 9 17 4 12"
  })), confirming ? '...' : lang === 'pt' ? 'Confirmar venda' : 'Confirm Sale'), onSkip && /*#__PURE__*/React.createElement("button", {
    onClick: onSkip,
    style: {
      display: 'block',
      width: '100%',
  ...
  }, lang === 'pt' ? 'Não vendi pelo app — pular' : lang === 'es' ? 'No lo vendí en la app — omitir' : "I didn't sell through the app — skip"), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    style: {
      width: '100%',
      marginTop: 10,
...
function ViewListingSheet({
  ...
  React.useEffect(() => {
    ...
    window.sb.from('rental_photos').select('type, photo_url').eq('request_id', myRequestId).then(({
      data
    }) => {
  ...
  const addPeriodToDate = (startDate, p, qty = 1) => {
    ...
    if (p === 'week') d.setDate(d.getDate() + 7 * qty);else if (p === 'month') d.setMonth(d.getMonth() + qty);else d.setDate(d.getDate() + qty);
    return d;
  };
  const fmtDate = d => {
    if (!d) return '';
    ...
    return dt.toLocaleDateString(lang === 'pt' ? 'pt-BR' : lang === 'es' ? 'es-ES' : 'en-US', {
      day: 'numeric',
      month: 'long'
    });
  };
  ...
  const handleRequestRental = async () => {
    ...
    const {
      data: inserted,
      error
    } = await window.sb.from('rental_requests').upsert({
      listing_id: item._id,
      listing_name: item.name || '',
  ...
  const handleOwnerDecision = async (requestId, newStatus) => {
    ...
    if (startDateStr) {
      updatePayload.start_date = startDateStr;
      updatePayload.end_date = endDateStr;
    }
    const {
      error
    } = await window.sb.from('rental_requests').update(updatePayload).eq('id', requestId);
    if (error) {
    ...
    if (req?.requester_id) {
      if (newStatus === 'approved') {
        ...
        _notify(req.requester_id, 'rental_approved', {
          ...
          en: `The owner approved your request for "${item.name || ''}". Return by ${dateLabel}.`,
          pt: `O dono aprovou seu pedido para "${item.name || ''}". Devolver até ${dateLabel}.`,
          es: `El propietario aprobó tu solicitud para "${item.name || ''}". Devolver antes del ${dateLabel}.`
        }, item._id);
      } else {
  ...
  const handleMarkReturned = async (requestId, req) => {
    ...
    const skipOk = window.confirm(lang === 'pt' ? 'Você não tirou fotos do estado inicial do item. Sem elas, não há como provar dano em caso de disputa. Marcar como devolvido mesmo assim?' : "You didn't take initial condition photos. Without them there's no proof in case of a damage dispute. Mark as returned anyway?");
    if (!skipOk) return;
    if (!window.sb) return;
    const {
      error
  ...
  const handleSubmitRating = async () => {
    ...
    window.sb.from('ratings').insert({
      ...
      to_id: currentUser.uid,
      from_name: ratingSheet.rateeName || '',
      listing_name: item.name || '',
      stars: null,
      comment: '',
      pending: true,
  ...
  const RequestRentalBlock = () => {
    ...
    if (reqStatus === 'approved') {
      ...
      }, lang === 'pt' ? 'O dono aprovou. Aproveite!' : 'The owner approved. Enjoy!'))), myEndDate && /*#__PURE__*/React.createElement("div", {
        style: {
          display: 'flex',
          alignItems: 'center',
          gap: 10,
      ...
      }, beforePics.length > 0 ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 11,
          fontWeight: 700,
          color: 'var(--pg-ink-500)',
      ...
      }, lang === 'pt' ? '📷 Estado do item documentado pelo dono antes da entrega:' : "📷 Item's condition documented by the owner before handoff:"), /*#__PURE__*/React.createElement("div", {
        style: {
          display: 'flex',
          gap: 6,
          flexWrap: 'wrap'
    ...
    if (reqStatus === 'completed') {
      ...
      }, hasRated ? lang === 'pt' ? 'Obrigado pela avaliação! ⭐' : 'Thanks for rating! ⭐' : lang === 'pt' ? 'Avalie sua experiência abaixo.' : 'Rate your experience below.'))), (beforePics.length > 0 || afterPics.length > 0) && /*#__PURE__*/React.createElement("div", {
        style: {
          padding: '10px 16px',
          background: 'var(--pg-white)',
      ...
      }, beforePics.length > 0 && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 10.5,
          fontWeight: 700,
          color: 'var(--pg-ink-500)',
      ...
      }, lang === 'pt' ? '📷 Antes' : '📷 Before'), /*#__PURE__*/React.createElement("div", {
        style: {
          display: 'flex',
          gap: 6,
          flexWrap: 'wrap'
      ...
      }, lang === 'pt' ? '📷 Depois' : '📷 After'), /*#__PURE__*/React.createElement("div", {
        style: {
          display: 'flex',
          gap: 6,
          flexWrap: 'wrap'
        }
      }, afterPics.map((url, i) => /*#__PURE__*/React.createElement("img", {
        key: i,
        src: url,
        alt: "",
        onClick: () => setRentalPhotoViewer({
    ...
    }, selPeriodEntry ? getPeriodLabel(selPeriodEntry.period, reqQty) : '')), selPeriodEntry && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11.5,
        color: 'var(--pg-ink-400)',
        marginTop: 6
      }
    }, lang === 'pt' ? `📅 Se aprovado hoje, devolução prevista até ${fmtDate(addPeriodToDate(new Date(), selPeriodEntry.period, reqQty))}` : lang === 'es' ? `📅 Si se aprueba hoy, devolución prevista antes del ${fmtDate(addPeriodToDate(new Date(), selPeriodEntry.period, reqQty))}` : `📅 If approved today, expected return by ${fmtDate(addPeriodToDate(new Date(), selPeriodEntry.period, reqQty))}`)), /*#__PURE__*/React.createElement("div", {
      style: {
        padding: '10px 12px',
        borderRadius: 12,
        background: 'var(--pg-ink-50)',
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        return /*#__PURE__*/React.createElement("div", {
          ...
          style: {
            padding: '9px 11px',
            background: hasBefore ? 'rgba(14,186,199,0.08)' : 'rgba(245,158,11,0.10)'
          }
        }, /*#__PURE__*/React.createElement("div", {
          style: {
            fontSize: 11.5,
            fontWeight: 800,
            color: hasBefore ? '#0EBAC7' : '#D97706'
          }
        }, hasBefore ? lang === 'pt' ? '📸 Estado inicial documentado' : '📸 Initial condition documented' : lang === 'pt' ? '📸 Etapa obrigatória: fotos do estado inicial' : '📸 Required step: initial condition photos'), !hasBefore && /*#__PURE__*/React.createElement("div", {
          style: {
            fontSize: 10.5,
            color: '#B45309',
            marginTop: 2,
        ...
        }, lang === 'pt' ? 'Tire fotos do item antes de entregar — protege você em caso de dano.' : 'Photograph the item before handing it over — protects you if it comes back damaged.')), hasBefore && /*#__PURE__*/React.createElement("div", {
          style: {
            display: 'flex',
            gap: 6,
            padding: '8px 11px',
        ...
        }, photoUploading && addingPhotoFor === req.id ? '⏳' : /*#__PURE__*/React.createElement(React.Fragment, null, "\uD83D\uDCF7 ", hasBefore ? lang === 'pt' ? 'Adicionar mais' : 'Add more' : lang === 'pt' ? 'Tirar fotos agora' : 'Take photos now')));
      })(), /*#__PURE__*/React.createElement("div", {
        style: {
          display: 'flex',
          gap: 6,
...
function MarketplaceScreen({
  ...
  const overdueSellListings = liveMarket.filter(m => isMyPost(m) && m.type === 'sell' && m.status === 'approved' && m.createdAt && Date.now() - new Date(m.createdAt).getTime() >= 15 * DAY_MS && (!m.lastReminderAt || Date.now() - new Date(m.lastReminderAt).getTime() >= 15 * DAY_MS));
  ...
  const snoozeReminder = item => {
    ...
    if (window.sb) window.sb.from('marketplace').update({
      last_reminder_at: new Date().toISOString()
    }).eq('id', item._id).catch(() => {});
  };
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem('pg_market_view', view);
    } catch (e) {}
  }, [view]);
  ...
  React.useEffect(() => {
    ...
    if (soon) {
      promptedMktExpiryRef.current.add(soon._id);
      setExpiringItem(soon);
    }
  ...
  React.useEffect(() => {
    if (!window.sb || !rentListingIdsKey) {
      setActiveRentalIds(new Set());
      return;
    }
    const ids = rentListingIdsKey.split(',');
    window.sb.from('active_rental_listing_ids').select('listing_id').in('listing_id', ids).then(({
      data
    }) => setActiveRentalIds(new Set((data || []).map(r => r.listing_id)))).catch(() => {});
  }, [rentListingIdsKey]);
  const tabIcons = {
  ...
  const renderLiveCard = (item, desktopMode = false) => {
    ...
    }, "SOLD"), isActiveRental && /*#__PURE__*/React.createElement("span", {
      style: {
        position: 'absolute',
        top: 10,
        left: 10,
    ...
    }, "\u23F3 ", lang === 'pt' ? 'EM ANDAMENTO' : lang === 'es' ? 'EN CURSO' : 'IN PROGRESS'), /*#__PURE__*/React.createElement("span", {
      style: {
        position: 'absolute',
        top: 10,
        right: 10,
  ...
  if (isDesktop) {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      points: "9 18 15 12 9 6"
    })))), visibleOverdue.length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        justifyContent: 'flex-end',
        padding: '10px 32px 0',
    ...
    }, visibleOverdue.length > 1 ? lang === 'pt' ? `${visibleOverdue.length} anúncios parados` : lang === 'es' ? `${visibleOverdue.length} anuncios sin movimiento` : `${visibleOverdue.length} listings idle` : lang === 'pt' ? 'Ainda está à venda?' : lang === 'es' ? '¿Sigue en venta?' : 'Still for sale?'), /*#__PURE__*/React.createElement("button", {
      onClick: () => openMyOrOthersListing(visibleOverdue[0]),
      style: {
        height: 26,
        padding: '0 10px',
    ...
    }, lang === 'pt' ? 'Ver' : lang === 'es' ? 'Ver' : 'View'), /*#__PURE__*/React.createElement("button", {
      onClick: () => snoozeReminder(visibleOverdue[0]),
      title: lang === 'pt' ? 'Manter anúncio' : lang === 'es' ? 'Mantener anuncio' : 'Keep listing',
      style: {
        width: 26,
        height: 26,
        borderRadius: '50%',
    ...
    })), /*#__PURE__*/React.createElement(ExpiringListingPrompt, {
      item: expiringItem,
      lang: lang,
      onClose: () => setExpiringItem(null),
      onRenew: renewListing,
      onNotAvailable: () => {
        setNotAvailableItem(expiringItem);
        setExpiringItem(null);
      }
    }), /*#__PURE__*/React.createElement(Sheet, {
      open: !!notAvailableItem,
      onClose: () => setNotAvailableItem(null),
      height: "auto"
    }, notAvailableItem && /*#__PURE__*/React.createElement(MarkSoldSheet, {
      ...
      onSold: sellerRating => {
        ...
        if (ctx && ctx.updateMarketItem) ctx.updateMarketItem(id, {
          status: 'sold'
        });
        openRating && sellerRating && openRating(sellerRating);
      },
      onSkip: async () => {
        const id = notAvailableItem._id;
        if (window.sb) await window.sb.from('marketplace').update({
          status: 'expired'
        }).eq('id', id);
        setNotAvailableItem(null);
        if (ctx && ctx.updateMarketItem) ctx.updateMarketItem(id, {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, lang === 'pt' ? visibleOverdue.length > 1 ? `${visibleOverdue.length} anúncios parados há 15+ dias` : 'Ainda está à venda?' : lang === 'es' ? visibleOverdue.length > 1 ? `${visibleOverdue.length} anuncios sin movimiento hace 15+ días` : '¿Sigue en venta?' : visibleOverdue.length > 1 ? `${visibleOverdue.length} listings idle 15+ days` : 'Still for sale?'), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: '#4338CA',
      marginTop: 2,
  ...
  }, lang === 'pt' ? `"${visibleOverdue[0].name || ''}" ainda não foi vendido` : lang === 'es' ? `"${visibleOverdue[0].name || ''}" todavía no se vendió` : `"${visibleOverdue[0].name || ''}" hasn't sold yet`)), /*#__PURE__*/React.createElement("svg", {
    width: "16",
    height: "16",
    viewBox: "0 0 24 24",
    fill: "none",
  ...
  }, lang === 'pt' ? 'Marcar como vendido' : lang === 'es' ? 'Marcar como vendido' : 'Mark as sold'), /*#__PURE__*/React.createElement("button", {
    onClick: () => snoozeReminder(visibleOverdue[0]),
    style: {
      flex: 1,
      height: 34,
  ...
  }, lang === 'pt' ? 'Manter anúncio' : lang === 'es' ? 'Mantener anuncio' : 'Keep listing'))), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '0 18px 16px'
    }
  }, /*#__PURE__*/React.createElement("div", {
    ...
    }, lang === 'pt' ? 'VENDIDO' : lang === 'es' ? 'VENDIDO' : 'SOLD') : isActiveRental ? /*#__PURE__*/React.createElement("span", {
      style: {
        position: 'absolute',
        top: 10,
        left: 10,
    ...
    }, "\u23F3 ", lang === 'pt' ? 'EM ANDAMENTO' : lang === 'es' ? 'EN CURSO' : 'IN PROGRESS') : isMyPost(item) && /*#__PURE__*/React.createElement("span", {
      style: {
        position: 'absolute',
        top: 10,
        left: 10,
