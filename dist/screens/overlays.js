...
function ChatConversation({
  ...
  const loadPendingVacApp = React.useCallback(() => {
    ...
    window.sb.from('job_applications').select('id,status,vacation_days').eq('job_id', rawId).eq('job_author_id', currentUser.uid).eq('applicant_id', convo.receiverId).eq('status', 'pending').limit(1).then(({
      data
    }) => setPendingVacApp(data && data[0] || null)).catch(() => setPendingVacApp(null));
  }, [currentUser?.uid, convo.receiverId, convo.listingId]);
  React.useEffect(() => {
    loadPendingVacApp();
  }, [loadPendingVacApp]);
  const handleVacDecision = async decision => {
    ...
    if (status === 'accepted' && selectedDays.length) {
      const {
        data: fresh
      } = await window.sb.from('vacations').select('booked_days').eq('id', rawId).single();
      freshBooked = fresh?.booked_days || [];
      const conflicts = selectedDays.filter(d => freshBooked.includes(d));
      if (conflicts.length > 0) {
        setVacActionBusy(false);
        setVacActionErr(lang === 'pt' ? `Conflito: o(s) dia(s) ${conflicts.join(', ')} já foram aceitos para outra pessoa. Resolva na tela de Candidatos.` : lang === 'es' ? `Conflicto: el/los día(s) ${conflicts.join(', ')} ya fueron aceptados para otra persona. Resuélvelo en la pantalla de Candidatos.` : `Conflict: day(s) ${conflicts.join(', ')} were already accepted for someone else. Resolve it from the Candidatos screen.`);
        return;
      }
    }
    const {
      ...
      status
    }).eq('id', pendingVacApp.id).eq('status', 'pending').select('id');
    ...
    setPendingVacApp(null);
    const msg = decision === 'approved' ? lang === 'pt' ? '✅ Candidatura para cobertura de férias aceita!' : lang === 'es' ? '✅ ¡Postulación de cobertura de vacaciones aceptada!' : '✅ Vacation coverage application accepted!' : lang === 'pt' ? '❌ Candidatura recusada.' : lang === 'es' ? '❌ Postulación rechazada.' : '❌ Application declined.';
    if (isLive) {
      window.sb.rpc('send_chat_message', {
        p_convo_id: convoId,
        p_body: msg,
    ...
    if (window.sb && convo.receiverId) {
      ...
      window.sb.from('notifications').insert({
        user_id: convo.receiverId,
        type: decision === 'approved' ? 'job_accepted' : 'job_rejected',
        title: JSON.stringify(title),
        body: msg,
        link_id: rawId,
        read: false
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, rentalActionDone === 'approved' ? lang === 'pt' ? '✅ Aluguel aprovado' : '✅ Rental approved' : lang === 'pt' ? '❌ Aluguel recusado' : '❌ Rental declined'), pendingVacApp && /*#__PURE__*/React.createElement("div", {
    style: {
      margin: '8px 12px 0',
      flexShrink: 0
    }
  }, vacActionErr && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11.5,
      color: '#DC2626',
      marginBottom: 6,
    ...
    onClick: () => handleVacDecision('approved'),
    disabled: vacActionBusy,
    style: {
      flex: 1,
      height: 38,
      borderRadius: 11,
      border: 'none',
      cursor: vacActionBusy ? 'default' : 'pointer',
      background: '#16A34A',
      color: '#fff',
      fontSize: 13,
      fontWeight: 700,
    ...
    points: "20 6 9 17 4 12"
  })), lang === 'pt' ? 'Aceitar candidatura' : lang === 'es' ? 'Aceptar postulación' : 'Accept application'), /*#__PURE__*/React.createElement("button", {
    onClick: () => handleVacDecision('declined'),
    disabled: vacActionBusy,
    style: {
      flex: 1,
      height: 38,
    ...
    x2: "18",
    y2: "18"
  })), lang === 'pt' ? 'Recusar' : lang === 'es' ? 'Rechazar' : 'Decline'))), vacActionDone && /*#__PURE__*/React.createElement("div", {
    style: {
      margin: '8px 12px 0',
      padding: '8px 12px',
      borderRadius: 11,
      ...
      color: vacActionDone === 'approved' ? '#16A34A' : '#EF4444',
      fontSize: 12.5,
      fontWeight: 700
    }
  }, vacActionDone === 'approved' ? lang === 'pt' ? '✅ Candidatura aceita' : lang === 'es' ? '✅ Postulación aceptada' : '✅ Application accepted' : lang === 'pt' ? '❌ Candidatura recusada' : lang === 'es' ? '❌ Postulación rechazada' : '❌ Application declined'), /*#__PURE__*/React.createElement("div", {
    ref: scroller,
    style: {
      flex: 1,
      overflow: 'auto',
...
function ApplicantsSheet({
  ...
  // Reporting a problem with vacation coverage — a Yes/No/Later checkpoint
  // ("did the pool guy finish?") in front of finalizing, mirroring the same
  // gate quickpools.jsx uses for Express Pools (YesNoLaterGate/
  // PhotoDisputeModal/COMPLETION_DISPUTE_REASONS live there and are globals
  // here by the time this actually renders — see build.js's script order).
  // "No" opens the reason-based report form; submitting it is best-effort
  // and always finalizes right after either way (see submitVacCompletionDispute) —
  // reporting a problem never leaves the coverage stuck open.
  const [completionGateFor, setCompletionGateFor] = React.useState(null); // applicant row
  const [completionReportFor, setCompletionReportFor] = React.useState(null); // applicant row
  const [completionDisputeBusy, setCompletionDisputeBusy] = React.useState(false);
  const submitVacCompletionDispute = async (reasonId, description) => {
    const a = completionReportFor;
    if (!reasonId || !description || !a) return;
    ...
    try {
      if (window.sb && user?.uid) {
        ...
        const evidenceUrls = (a.submittedPhotos || []).map(p => p.url).filter(Boolean);
        const {
          error
        ...
        if (error && !dup) showToast && showToast('❌ ' + (error.message || 'Error'));else showToast && showToast(dup ? lang === 'pt' ? '⚠ Você já tem uma reclamação aberta sobre esta cobertura.' : lang === 'es' ? '⚠ Ya tienes un reclamo abierto sobre esta cobertura.' : '⚠ You already have an open report on this coverage.' : lang === 'pt' ? '✅ Problema reportado.' : lang === 'es' ? '✅ Problema reportado.' : '✅ Problem reported.');
      }
    } catch (e) {
      showToast && showToast('❌ ' + (e?.message || 'Error'));
    } finally {
      setCompletionDisputeBusy(false);
      setCompletionReportFor(null);
      finalizeVacation(a); // best-effort report — never blocks finalizing
    }
  };
  ...
  const dbUpdate = async (appDbId, patch) => {
    ...
    if (error) {
      console.warn('[ApplicantsSheet] update error:', error.message);
  ...
  const updateStatus = async (id, status, extra = {}) => {
    ...
    if (post?._live && app?._dbId) {
      ...
      if (status === 'accepted' && post.type === 'vacation' && app.selectedDays?.length) {
        ...
        if (conflicts.length > 0) {
          showToast && showToast('❌ ' + (lang === 'pt' ? `Conflito: dia(s) ${conflicts.join(', ')} já foram aceitos para outra pessoa.` : lang === 'es' ? `Conflicto: día(s) ${conflicts.join(', ')} ya fueron aceptados para otra persona.` : `Conflict: day(s) ${conflicts.join(', ')} were already accepted for someone else.`));
          loadLiveApplicants(post._id);
          return;
        }
      }
      ...
      const {
        ...
        status,
        ...extra
      }).eq('id', app._dbId).eq('status', app.status).select('id');
      if (updErr) {
    ...
    if (a.applicant_id && window.sb) {
      window.sb.from('notifications').insert({
        ...
        title: JSON.stringify({
          ...
          pt: 'Cobertura confirmada ✓',
          es: 'Cobertura confirmada ✓'
        }),
        body: JSON.stringify({
      ...
      window.sendPush && window.sendPush(a.applicant_id, lang === 'pt' ? '✅ Cobertura confirmada' : lang === 'es' ? '✅ Cobertura confirmada' : '✅ Coverage confirmed', lang === 'pt' ? `${post.author || 'O dono'} confirmou sua cobertura de férias.` : lang === 'es' ? `${post.author || 'El dueño'} confirmó tu cobertura de vacaciones.` : `${post.author || 'The owner'} confirmed your vacation coverage.`, '/#work', 'work');
    }
  ...
  return /*#__PURE__*/React.createElement(Sheet, {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "\uD83D\uDCC5 ", tr(a.interview.day, lang), " \xB7 ", a.interview.time)), post.type === 'vacation' && a.status === 'accepted' && a.poolGuyDone && /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 10,
        padding: '10px 12px',
        borderRadius: 10,
        background: 'oklch(0.96 0.04 260)',
        border: '0.5px solid oklch(0.85 0.08 260)'
      }
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11.5,
      ...
      onClick: () => setCompletionGateFor(a),
    ...
    }, Icon.check(14, '#fff'), " ", lang === 'pt' ? 'Finalizar e avaliar' : lang === 'es' ? 'Finalizar y calificar' : 'Finalize and rate')), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 7,
        marginTop: 11,
  ...
  }), /*#__PURE__*/React.createElement(YesNoLaterGate, {
    open: !!completionGateFor,
    ...
    lang: lang,
    onYes: () => {
      const a = completionGateFor;
      setCompletionGateFor(null);
      finalizeVacation(a);
    },
    onLater: () => {
      const a = completionGateFor;
    ...
    onNo: () => setCompletionReportFor(completionGateFor)
  }), /*#__PURE__*/React.createElement(PhotoDisputeModal, {
    open: !!completionReportFor,
    ...
    intro: lang === 'pt' ? 'A cobertura finaliza normalmente — isso só registra o problema para nossa equipe revisar.' : lang === 'es' ? 'La cobertura finaliza normalmente — esto solo registra el problema para que nuestro equipo lo revise.' : "The coverage still finishes normally — this just logs the problem for our team to review.",
    photos: completionReportFor?.submittedPhotos || [],
    busy: completionDisputeBusy,
    onSubmit: submitVacCompletionDispute,
    onClose: () => {
      const a = completionReportFor;
      setCompletionReportFor(null);
      if (a) finalizeVacation(a);
    },
    lang: lang
  }));
}
...
function NotificationsSheet({
  ...
  React.useEffect(() => {
    if (!open || !user?.uid || !window.sb) return;
    const check = () => window.sb.from('notifications').select('*').eq('user_id', user.uid).order('created_at', {
      ascending: false
    }).limit(10).then(({
      data
    }) => {
  ...
  const iconFor = type => {
    ...
    if (type === 'rating' || type === 'rating_revealed') return /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 16
      }
    }, "\u2B50");
    ...
    if (type === 'vacation_photos_submitted') return /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 16
      }
    }, "\uD83D\uDCF8");
...
function PaywallSheet({
  ...
  const ROWS = [{
    ...
    free: lang === 'pt' ? '5/semana' : lang === 'es' ? '5/semana' : '5/week',
    pro: lang === 'pt' ? 'Ilimitado' : lang === 'es' ? 'Ilimitado' : 'Unlimited',
    premium: lang === 'pt' ? 'Ilimitado' : lang === 'es' ? 'Ilimitado' : 'Unlimited',
    type: 'count'
  }, {
  ...
  }, Icon.bolt(13, 'var(--pg-blue-700)'), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 12,
      color: 'var(--pg-blue-700)',
      lineHeight: 1.4
    }
  }, lang === 'pt' ? 'Você atingiu o limite de 5 Piscinas Rápidas publicadas essa semana no plano grátis. O limite reseta todo domingo às 6h — ou vire PRO pra publicar sem limite.' : lang === 'es' ? 'Alcanzaste el límite de 5 Piscinas Rápidas publicadas esta semana en el plan gratis. El límite se reinicia cada domingo a las 6am — o hazte PRO para publicar sin límite.' : "You've hit the free plan's limit of 5 Quick Pools posted this week. It resets every Sunday at 6am — or go PRO to post unlimited."))), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '0 16px 14px',
      display: 'flex',
      gap: 8
...
function WalletSheet({
  ...
  const L = lang === 'pt' ? {
    ...
    referral: 'Indicações',
    history: 'Histórico',
    invite: 'Indique e ganhe',
    inviteSub: 'Compartilhe seu link. Quando a pessoa assinar, você ganha.',
    yourLink: 'Seu link',
    copy: 'Copiar link',
    copied: 'Link copiado!',
    share: 'Compartilhar',
    ...
    referral: 'Referidos',
    history: 'Historial',
    invite: 'Refiere y gana',
    inviteSub: 'Comparte tu enlace. Cuando la persona se suscriba, tú ganas.',
    yourLink: 'Tu enlace',
    copy: 'Copiar enlace',
    ...
    min: `Mínimo de ${money(WITHDRAW_MIN_CENTS)} para retirar`,
    pending: 'retiro en revisión',
    ...
    history: 'History',
    invite: 'Refer & earn',
    inviteSub: 'Share your link. When they subscribe, you earn.',
    yourLink: 'Your link',
    copy: 'Copy link',
    copied: 'Link copied!',
    share: 'Share',
    ...
    min: `${money(WITHDRAW_MIN_CENTS)} minimum to withdraw`,
  ...
  const shareLink = async () => {
    if (!link) return;
    const text = lang === 'pt' ? `Entra no PoolGuyX comigo e ganha desconto na assinatura: ${link}` : lang === 'es' ? `Únete a PoolGuyX conmigo y obtén descuento en la suscripción: ${link}` : `Join me on PoolGuyX and get a discount on your subscription: ${link}`;
    if (navigator.share) {
      try {
  ...
  const submitWithdraw = async () => {
    ...
    try {
      const {
        data,
        error
      } = await window.sb.rpc('request_withdrawal', {
        p_amount_cents: cents
      });
      if (error || !data) {
        showToast && showToast('❌ ' + L.errGeneric);
        return;
      }
      if (!data.ok) {
        showToast && showToast('❌ ' + (data.error === 'below_minimum' ? L.errMin : data.error === 'insufficient_funds' ? L.errFunds : L.errGeneric));
        return;
      }
      setWdOpen(false);
      setWdAmount('');
  ...
  }, L.wallet), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    style: {
      border: 'none',
      background: 'var(--pg-ink-100)',
  ...
  }, L.balance.toUpperCase()), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: 'var(--pg-font-display)',
      fontSize: 46,
      fontWeight: 700,
  ...
  }, money(balance)), pendingWd > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 8,
      fontSize: 11.5,
      opacity: 0.75
  ...
  }, L.invited.toUpperCase()), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 17,
      fontWeight: 700
    }
  }, referred)), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 9,
      opacity: 0.6,
      fontWeight: 700,
  ...
  }, L.converted.toUpperCase()), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 17,
      fontWeight: 700
    }
  }, converted)), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 9,
      opacity: 0.6,
      fontWeight: 700,
  ...
  }, L.earned.toUpperCase()), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 17,
      fontWeight: 700
    }
  }, money(earned))))), /*#__PURE__*/React.createElement("button", {
  ...
  }, ArrowUp(16, 'var(--pg-blue-900)'), " ", L.withdraw), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: 'var(--pg-ink-500)',
      textAlign: 'center',
  ...
  }, L.min), /*#__PURE__*/React.createElement("div", {
    className: "pg-seg",
    style: {
      marginBottom: 14
    }
  }, /*#__PURE__*/React.createElement("button", {
    className: `pg-seg-btn ${tab === 'referral' ? 'on' : ''}`,
    onClick: () => setTab('referral')
  }, L.referral), /*#__PURE__*/React.createElement("button", {
    className: `pg-seg-btn ${tab === 'history' ? 'on' : ''}`,
    onClick: () => setTab('history')
  }, L.history)), tab === 'referral' && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 14
    }
  }, myDisc && /*#__PURE__*/React.createElement("div", {
    style: {
      borderRadius: 14,
      padding: '12px 14px',
      background: 'rgba(14,186,199,0.10)',
  ...
  }, L.youGetSub)), /*#__PURE__*/React.createElement("div", {
    className: "pg-card",
    style: {
      padding: '14px 15px'
    }
  ...
  }, L.inviteSub), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
      fontWeight: 700,
      color: 'var(--pg-ink-500)',
  ...
  }, L.yourLink.toUpperCase()), /*#__PURE__*/React.createElement("input", {
    id: "pgRefLinkInput",
    readOnly: true,
    value: link,
  ...
  }, copied ? '✓ ' + L.copied : L.copy), /*#__PURE__*/React.createElement("button", {
    onClick: shareLink,
    disabled: !link,
    style: {
      flex: 1,
  ...
  }, L.share))), /*#__PURE__*/React.createElement("div", {
    className: "pg-card",
    style: {
      padding: '14px 15px'
    }
  ...
  }, L.table), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
  ...
  }, L.monthly.toUpperCase()), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
      fontWeight: 700,
      color: 'var(--pg-ink-500)',
  ...
  }, L.annual.toUpperCase()), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      fontWeight: 600,
      color: '#0077B6'
  ...
  }, L.noRef)), tab === 'history' && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 1
    }
  }, walletTx.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: 'center',
      padding: '30px 16px',
      color: 'var(--pg-ink-400)',
  ...
  }, L.askAmount), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: 'var(--pg-ink-500)',
      marginBottom: 14
    }
  }, L.balance, ": ", money(balance), " \xB7 ", L.min), /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'relative',
      marginBottom: 14
    }
...
function RatingSheet({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (rating.to_id) {
        const bodyObj = {
          en: `${myName} rated you! Rate them back.`,
          pt: `${myName} avaliou você! Avalie-o também.`,
          es: `${myName} te calificó! Califícalo también.`
        };
        window.sb.from('notifications').insert({
          user_id: rating.to_id,
          type: 'rating',
          title: JSON.stringify({
            en: 'You were rated! ⭐',
            pt: 'Você foi avaliado! ⭐',
...
function TermsSheet({
  ...
  const sections = lang === 'pt' ? [{
    title: '1. Usando o PoolGuyX',
    body: 'O PoolGuyX é uma plataforma onde profissionais e donos de piscina se encontram: anúncios de equipamentos, aluguéis, trabalhos de Piscinas Rápidas, cobertura de férias, rotas e vagas. Ao criar uma conta, você confirma que tem 18 anos ou mais e concorda com estes Termos e com a Política de Privacidade.'
  }, {
    title: '2. Somos uma plataforma, não parte do negócio',
    body: 'O PoolGuyX não contrata, não emprega, não segura nem supervisiona ninguém, e não é parte de nenhuma venda, aluguel, trabalho ou serviço combinado entre usuários. Cada usuário é independente e responsável pelo próprio trabalho, segurança, qualidade e resultado.'
  }, {
    title: '3. Pagamentos entre usuários',
    body: 'Pagamentos de vendas, aluguéis e trabalhos são combinados e feitos diretamente entre as pessoas envolvidas, fora do app. O PoolGuyX não guarda, garante, reembolsa nem intermedia esse dinheiro, e não existe custódia (escrow). Só pague e receba de quem você confia e guarde seus próprios comprovantes. Os planos pagos, quando disponíveis, são processados pelo Stripe.'
  }, {
    title: '4. Licenças, seguro e impostos',
    body: 'Você é responsável por licenças, seguros, autorizações e impostos exigidos pelo seu trabalho (por exemplo, regras da Flórida para contratados e manuseio de produtos químicos) e por cumprir todas as leis aplicáveis.'
  }, {
    title: '5. Seu conteúdo',
    body: 'Você é dono das fotos e textos que publica e nos autoriza a exibi-los no app para o serviço funcionar. Publique apenas o que você tem direito de compartilhar. Fotos enviadas como prova de serviço ou como evidência podem ser vistas pela outra parte e pelos administradores do PoolGuyX.'
  }, {
    title: '6. Avaliações e denúncias',
    body: 'As avaliações devem ser honestas e baseadas em experiência real. Você pode denunciar um problema (por exemplo, serviço não pago ou trabalho não concluído). As denúncias vão para os administradores do PoolGuyX, que podem analisar as evidências e decidir o que fazer, inclusive advertência ou suspensão. Não somos obrigados a mediar disputas nem a recuperar dinheiro, e nossas decisões são tomadas a nosso critério.'
  }, {
    title: '7. O que não é permitido',
    body: 'Fraude, anúncios ou avaliações falsas, assédio, discriminação, itens ou serviços ilegais, spam, divulgação de dados privados de terceiros, tentar quebrar ou burlar o app, ou usá-lo para qualquer fim ilegal.'
  }, {
    title: '8. Suspensão e exclusão',
    body: 'Podemos limitar ou suspender contas que descumprirem estas regras. Você pode excluir sua conta a qualquer momento em Perfil → Excluir minha conta.'
  }, {
    title: '9. Versão beta',
    body: 'O PoolGuyX está em fase beta. É oferecido “como está” e “conforme disponível”: recursos podem mudar e pode haver erros ou interrupções. Trabalhamos para proteger seus dados, mas não podemos garantir que nada será perdido.'
  }, {
    title: '10. Limite de responsabilidade',
    body: 'Na extensão máxima permitida por lei, o PoolGuyX não responde por disputas entre usuários, trabalhos não pagos, equipamentos danificados, lesões ou perdas indiretas decorrentes do uso do app. Nada aqui limita direitos que a lei não permite limitar.'
  }, {
    title: '11. Mudanças e lei aplicável',
    body: 'Podemos atualizar estes Termos; se a mudança for relevante, avisaremos no app. Continuar usando o app significa aceitar a atualização. Estes Termos são regidos pelas leis do Estado da Flórida, EUA.'
  }, {
    title: '12. Contato',
    body: 'Dúvidas ou pedidos: use Perfil → Ajuda e suporte ou Enviar Feedback dentro do app.'
  }] : lang === 'es' ? [{
    title: '1. Usar PoolGuyX',
    body: 'PoolGuyX es una plataforma donde profesionales y dueños de piscinas se encuentran: anuncios de equipos, alquileres, trabajos de Piscinas Rápidas, cobertura de vacaciones, rutas y vacantes. Al crear una cuenta confirmas que tienes 18 años o más y aceptas estos Términos y la Política de Privacidad.'
  }, {
    title: '2. Somos una plataforma, no parte del negocio',
    body: 'PoolGuyX no contrata, no emplea, no asegura ni supervisa a nadie, y no es parte de ninguna venta, alquiler, trabajo o servicio acordado entre usuarios. Cada usuario es independiente y responsable de su propio trabajo, seguridad, calidad y resultado.'
  }, {
    title: '3. Pagos entre usuarios',
    ...
    title: '4. Licencias, seguros e impuestos',
    body: 'Eres responsable de las licencias, seguros, permisos e impuestos que exija tu trabajo (por ejemplo, las reglas de Florida para contratistas y manejo de químicos) y de cumplir todas las leyes aplicables.'
  }, {
    title: '5. Tu contenido',
    body: 'Eres dueño de las fotos y textos que publicas y nos autorizas a mostrarlos en la app para que el servicio funcione. Publica solo lo que tienes derecho a compartir. Las fotos enviadas como prueba de trabajo o evidencia pueden ser vistas por la otra parte y por los administradores de PoolGuyX.'
  }, {
    title: '6. Calificaciones y denuncias',
    body: 'Las calificaciones deben ser honestas y basadas en experiencia real. Puedes denunciar un problema (por ejemplo, un trabajo no pagado o sin terminar). Las denuncias van a los administradores de PoolGuyX, que pueden revisar las pruebas y decidir qué hacer, incluso advertencia o suspensión. No estamos obligados a mediar disputas ni a recuperar dinero, y nuestras decisiones son a nuestro criterio.'
  }, {
    title: '7. Qué no está permitido',
    body: 'Fraude, anuncios o reseñas falsas, acoso, discriminación, artículos o servicios ilegales, spam, compartir datos privados de terceros, intentar romper o eludir la app, o usarla para cualquier fin ilegal.'
  }, {
    title: '8. Suspensión y eliminación',
    body: 'Podemos limitar o suspender cuentas que incumplan estas reglas. Puedes eliminar tu cuenta en cualquier momento en Perfil → Eliminar mi cuenta.'
  }, {
    title: '9. Versión beta',
    body: 'PoolGuyX está en fase beta. Se ofrece “tal cual” y “según disponibilidad”: las funciones pueden cambiar y puede haber errores o interrupciones. Trabajamos para proteger tus datos, pero no podemos garantizar que nada se pierda.'
  }, {
    title: '10. Límite de responsabilidad',
    body: 'En la máxima medida permitida por la ley, PoolGuyX no responde por disputas entre usuarios, trabajos no pagados, equipos dañados, lesiones o pérdidas indirectas derivadas del uso de la app. Nada aquí limita derechos que la ley no permite limitar.'
  }, {
    title: '11. Cambios y ley aplicable',
    body: 'Podemos actualizar estos Términos; si el cambio es importante, te avisaremos en la app. Seguir usando la app significa aceptar la actualización. Estos Términos se rigen por las leyes del Estado de Florida, EE. UU.'
  }, {
    title: '12. Contacto',
    body: 'Dudas o solicitudes: usa Perfil → Ayuda y soporte o Enviar Feedback dentro de la app.'
  }] : [{
    title: '1. Using PoolGuyX',
    body: 'PoolGuyX is a platform where pool professionals and pool owners find each other: equipment listings, rentals, Express Pools jobs, vacation coverage, routes and job openings. By creating an account you confirm you are at least 18 and agree to these Terms and the Privacy Policy.'
  }, {
    title: '2. We are a platform, not a party to the deal',
    body: 'PoolGuyX does not hire, employ, insure or supervise anyone, and is not a party to any sale, rental, job or service agreed between users. Every user is independent and responsible for their own work, safety, quality and results.'
  }, {
    title: '3. Payments between users',
    body: 'Payments for sales, rentals and jobs are agreed and made directly between the people involved, outside the app. PoolGuyX does not hold, guarantee, refund or mediate that money, and there is no escrow. Only pay and get paid with people you trust, and keep your own records. Paid plans, when available, are processed by Stripe.'
  }, {
    title: '4. Licenses, insurance and taxes',
    body: 'You are responsible for the licenses, insurance, permits and taxes your work requires (for example Florida contractor and chemical-handling rules) and for following all applicable laws.'
  }, {
    title: '5. Your content',
    body: 'You own the photos and text you post, and you allow us to display them in the app so the service works. Only post what you have the right to share. Photos sent as proof of work or as evidence may be seen by the other party and by PoolGuyX administrators.'
  }, {
    title: '6. Ratings and reports',
    body: 'Ratings must be honest and based on real experience. You can report a problem (for example an unpaid or unfinished job). Reports go to PoolGuyX administrators, who may review the evidence and decide what to do, including a warning or suspension. We are not obliged to mediate disputes or recover money, and our decisions are made at our discretion.'
  }, {
    title: '7. What is not allowed',
    body: 'Fraud, fake listings or reviews, harassment, discrimination, illegal items or services, spam, sharing other people’s private data, trying to break or bypass the app, or using it for anything unlawful.'
  }, {
    title: '8. Suspension and deletion',
    body: 'We may limit or suspend accounts that break these rules. You can delete your account at any time in Profile → Delete my account.'
  }, {
    title: '9. Beta service',
    body: 'PoolGuyX is in beta. It is provided “as is” and “as available”: features may change and there may be errors or interruptions. We work to protect your data but cannot guarantee that nothing will be lost.'
  }, {
    title: '10. Limit of liability',
    body: 'To the fullest extent allowed by law, PoolGuyX is not liable for disputes between users, unpaid work, damaged equipment, injuries or indirect losses arising from use of the app. Nothing here limits rights that cannot be limited by law.'
  }, {
    title: '11. Changes and governing law',
    body: 'We may update these Terms; if the change is material we will tell you in the app. Continuing to use the app means you accept the update. These Terms are governed by the laws of the State of Florida, USA.'
  }, {
    title: '12. Contact',
    body: 'Questions or requests: use Profile → Help & support or Send Feedback inside the app.'
  }];
  return /*#__PURE__*/React.createElement("div", {
    className: "pg-sheet-backdrop",
    onClick: onClose,
...
function PrivacySheet({
  ...
  const sections = lang === 'pt' ? [{
    ...
    body: 'Você pode excluir sua conta e seus dados a qualquer momento em Perfil → Excluir minha conta. Depois disso, seu perfil, anúncios, candidaturas e conversas são removidos. Registros de denúncias podem ser mantidos para segurança dos outros usuários.'
  }] : lang === 'es' ? [{
    ...
    body: 'Puedes eliminar tu cuenta y tus datos en cualquier momento en Perfil → Eliminar mi cuenta. Después se eliminan tu perfil, anuncios, postulaciones y chats. Los registros de denuncias pueden conservarse por la seguridad de otros usuarios.'
  }] : [{
    ...
    body: 'You can delete your account and data at any time in Profile → Delete my account. Your profile, listings, applications and chats are then removed. Records of reports may be kept to protect other users.'
  }];
  return /*#__PURE__*/React.createElement("div", {
    className: "pg-sheet-backdrop",
    onClick: onClose
...
function BuyerRatingPromptModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (rating.from_id) {
        const bodyObj = {
          en: `${myName} also rated you! Both ratings are now visible.`,
          pt: `${myName} também te avaliou! Ambas as avaliações agora estão visíveis.`,
          es: `${myName} también te calificó! Ambas calificaciones ya son visibles.`
        };
        window.sb.from('notifications').insert({
          user_id: rating.from_id,
