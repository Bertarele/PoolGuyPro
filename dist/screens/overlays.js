...
function ApplicantsSheet({
  ...
  const submitVacationReport = async a => {
    ...
    if (error) {
      showToast && showToast((error.message || '').includes('one_open_per_reporter') ? lang === 'pt' ? '⚠ Você já tem uma reclamação aberta sobre esta cobertura.' : lang === 'es' ? '⚠ Ya tienes un reclamo abierto sobre esta cobertura.' : '⚠ You already have an open report on this coverage.' : '❌ ' + (error.message || 'Error'));
      return;
    }
    setReportingFor(null);
    setReportDesc('');
    setReportSev('serious');
    showToast && showToast(lang === 'pt' ? '⚠ Problema reportado e enviado para análise.' : lang === 'es' ? '⚠ Problema reportado y enviado a revisión.' : '⚠ Issue reported and sent for review.');
  };
  ...
  return /*#__PURE__*/React.createElement(Sheet, {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, Icon.check(14, '#fff'), " ", lang === 'pt' ? 'Finalizar e avaliar' : lang === 'es' ? 'Finalizar y calificar' : 'Finalize and rate'), /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        setReportingFor(reportingFor === a.id ? null : a.id);
        setReportDesc('');
      },
      style: {
        flex: 1,
    ...
    }, "\u26A0 ", lang === 'pt' ? 'Problema' : lang === 'es' ? 'Problema' : 'Issue')), reportingFor === a.id && /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 10,
        padding: '12px',
        borderRadius: 10,
    ...
    }, lang === 'pt' ? 'O que aconteceu?' : lang === 'es' ? '¿Qué pasó?' : 'What happened?'), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 6,
        marginBottom: 9
      ...
      es: 'Crítico',
      en: 'Critical'
    }].map(sv => /*#__PURE__*/React.createElement("button", {
      key: sv.id,
      onClick: () => setReportSev(sv.id),
      style: {
        flex: 1,
        height: 30,
        borderRadius: 8,
    ...
    }, sv[lang] || sv.en))), /*#__PURE__*/React.createElement("textarea", {
      value: reportDesc,
      onChange: e => setReportDesc(e.target.value),
      rows: 3,
      placeholder: lang === 'pt' ? 'Descreva o problema — piscinas sujas, dias não feitos, fotos que não batem…' : lang === 'es' ? 'Describe el problema — piscinas sucias, días no hechos, fotos que no coinciden…' : 'Describe the problem — dirty pools, days skipped, photos that do not match…',
      style: {
        width: '100%',
        padding: '9px 11px',
        borderRadius: 9,
      ...
      onClick: () => submitVacationReport(a),
      disabled: !reportDesc.trim() || reportSaving,
      style: {
        width: '100%',
        height: 36,
        marginTop: 9,
        borderRadius: 999,
        border: 'none',
        cursor: !reportDesc.trim() || reportSaving ? 'default' : 'pointer',
        fontFamily: 'inherit',
        fontSize: 12.5,
        fontWeight: 700,
        color: '#fff',
    ...
    }, reportSaving ? '…' : lang === 'pt' ? 'Enviar para análise' : lang === 'es' ? 'Enviar a revisión' : 'Send for review'), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 10.5,
        color: 'var(--pg-ink-400)',
        marginTop: 7,
    ...
    }, lang === 'pt' ? 'A cobertura fica em aberto até a equipe analisar. Você ainda pode finalizar depois.' : lang === 'es' ? 'La cobertura queda abierta hasta la revisión. Aún puedes finalizar después.' : 'The coverage stays open until the team reviews it. You can still finalize later.'))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 7,
        marginTop: 11,
