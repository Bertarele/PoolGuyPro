...
const PHOTO_DISPUTE_REASONS = [{
  ...
  en: 'Low quality',
  es: 'Baja calidad',
  dPt: 'Foto borrada, escura ou cortada',
  dEn: 'Blurry, dark or cropped photo',
  dEs: 'Foto borrosa, oscura o cortada'
}, {
  id: 'wrong_type',
  sev: 'minor',
  emoji: '🔀',
  pt: 'Tipo de foto errado',
  en: 'Wrong photo type',
  es: 'Tipo de foto incorrecto',
  dPt: 'Pedi um ângulo/parte, veio outro',
  dEn: 'Asked for one angle/part, got another',
  dEs: 'Pedí un ángulo/parte, llegó otro'
}, {
  id: 'mismatch',
  sev: 'serious',
  emoji: '❓',
  pt: 'Não bate com a piscina',
  en: "Doesn't match the pool",
  es: 'No coincide con la piscina',
  dPt: 'Parece outro lugar',
  dEn: 'Looks like a different place',
  dEs: 'Parece otro lugar'
}, {
  id: 'missing_angle',
  sev: 'minor',
  emoji: '📐',
  pt: 'Faltou uma foto obrigatória',
  en: 'Missing a required photo',
  es: 'Falta una foto obligatoria',
  dPt: 'Uma das fotos pedidas não veio',
  dEn: "One of the required photos wasn't sent",
  dEs: 'Una de las fotos pedidas no llegó'
}, {
  id: 'other',
  sev: 'minor',
  emoji: '✏️',
...
const PAYMENT_DISPUTE_REASONS = [{
  ...
  emoji: '🚫',
  pt: 'Não recebi nada',
  en: "Didn't get paid at all",
  es: 'No recibí nada',
  dPt: 'Serviço feito, sem pagamento até agora',
  dEn: 'Job done, no payment so far',
  dEs: 'Servicio hecho, sin pago hasta ahora'
}, {
  id: 'partial',
  sev: 'minor',
  emoji: '➗',
  pt: 'Recebi só uma parte',
  en: 'Only got part of it',
  es: 'Recibí solo una parte',
  dPt: 'Combinamos um valor, veio menos',
  dEn: 'Agreed on an amount, got less',
  dEs: 'Acordamos un monto, llegó menos'
}, {
  id: 'wrong_amount',
  sev: 'minor',
  emoji: '🔢',
  pt: 'Valor diferente do combinado',
  en: 'Amount different from agreed',
  es: 'Monto distinto al acordado',
  dPt: 'Pagou, mas não o valor certo',
  dEn: 'Paid, but not the right amount',
  dEs: 'Pagó, pero no el monto correcto'
}, {
  id: 'other',
  sev: 'minor',
  emoji: '✏️',
...
const COMPLETION_DISPUTE_REASONS = [{
  ...
  en: "Never showed up",
  es: 'Nunca apareció',
  dPt: 'Marcado, mas não foi feito',
  dEn: "Scheduled but never done",
  dEs: 'Programado pero nunca se hizo'
}, {
  id: 'not_done',
  sev: 'serious',
  emoji: '🏊',
  pt: 'Piscina não foi limpa',
  en: "Pool wasn't cleaned",
  es: 'La piscina no fue limpiada',
  dPt: 'Fotos ou não, o serviço não foi feito',
  dEn: "Photos or not, the work wasn't done",
  dEs: 'Fotos o no, el trabajo no se hizo'
}, {
  id: 'partial_job',
  sev: 'minor',
  emoji: '🌓',
  pt: 'Serviço pela metade',
  en: 'Job half-done',
  es: 'Trabajo a medias',
  dPt: 'Começou mas não terminou direito',
  dEn: "Started but didn't finish properly",
  dEs: 'Empezó pero no terminó bien'
}, {
  id: 'other',
  sev: 'minor',
  emoji: '✏️',
...
function PhotoDisputeModal({
  open,
  reasons = PHOTO_DISPUTE_REASONS,
  title,
  intro,
  questionLabel,
  photos = [],
  ...
  const addUploads = fileList => {
    ...
    const next = Array.from(fileList || []).slice(0, room).map(file => ({
      file,
  ...
  const introText = intro || (lang === 'pt' ? 'A vaga finaliza normalmente — isso só registra o problema para nossa equipe revisar.' : lang === 'es' ? 'La vacante finaliza normalmente — esto solo registra el problema para que nuestro equipo lo revise.' : "The job still finishes normally — this just logs the problem for our team to review.");
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, introText), photos.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8,
      flexWrap: 'wrap',
  ...
  }, r.emoji), /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13.5,
      fontWeight: 700,
      color: reason === r.id ? '#DC2626' : 'var(--pg-ink-800)'
    }
  }, label(r)), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: 'var(--pg-ink-400)',
      marginTop: 1
    }
  }, descOf(r)))))), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      fontWeight: 700,
      color: 'var(--pg-ink-700)',
  ...
  }, lang === 'pt' ? 'Detalhes:' : lang === 'es' ? 'Detalles:' : 'Details:'), /*#__PURE__*/React.createElement("textarea", {
    value: desc,
    ...
    style: {
      ...
      outline: 'none',
      marginBottom: allowUpload ? 16 : 18,
      display: 'block'
    }
  }), allowUpload && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      fontWeight: 700,
      color: 'var(--pg-ink-700)',
  ...
  }, "\uD83D\uDCCE ", lang === 'pt' ? `Provas — print, comprovante etc. (${uploads.length}/5)` : lang === 'es' ? `Pruebas — captura, comprobante, etc. (${uploads.length}/5)` : `Evidence — screenshot, receipt, etc. (${uploads.length}/5)`), uploads.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8,
      flexWrap: 'wrap',
  ...
  }, "\xD7")))), uploads.length < 5 && /*#__PURE__*/React.createElement("label", {
    style: {
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
  ...
  })), lang === 'pt' ? 'Adicionar prova' : lang === 'es' ? 'Agregar prueba' : 'Add evidence', /*#__PURE__*/React.createElement("input", {
    type: "file",
    accept: "image/*",
    multiple: true,
    style: {
    ...
    disabled: !reason || !desc.trim() || busy || uploading,
    style: {
      width: '100%',
      height: 50,
      borderRadius: 14,
      ...
      opacity: !reason || !desc.trim() ? 0.6 : 1,
      transition: 'all .15s'
    }
  }, busy || uploading ? lang === 'pt' ? 'Enviando...' : lang === 'es' ? 'Enviando...' : 'Sending...' : lang === 'pt' ? 'Reportar e finalizar' : lang === 'es' ? 'Reportar y finalizar' : 'Report & finish'), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    style: {
      width: '100%',
      height: 42,
...
function YesNoLaterGate({
  ...
  }, lang === 'pt' ? 'Não' : lang === 'es' ? 'No' : 'No'), /*#__PURE__*/React.createElement("button", {
    onClick: onYes,
    style: {
      flex: 1,
      height: 50,
  ...
  }, lang === 'pt' ? 'Sim' : lang === 'es' ? 'Sí' : 'Yes')), /*#__PURE__*/React.createElement("button", {
    onClick: onLater,
    style: {
      width: '100%',
      height: 42,
...
function PendingRatingModal({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, lang === 'pt' ? 'Avaliação pendente' : lang === 'es' ? 'Calificación pendiente' : 'Pending rating'), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 14,
      color: 'var(--pg-ink-500)',
      textAlign: 'center',
  ...
  }, target.listing_name || (lang === 'pt' ? 'Serviço concluído' : 'Completed job')), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: 'var(--pg-blue-700)',
      background: 'var(--pg-blue-50)',
  ...
  }, lang === 'pt' ? 'Você só verá a avaliação da outra pessoa depois de avaliar.' : lang === 'es' ? 'Solo verás la calificación de la otra persona después de calificar.' : "You'll only see their rating of you after you rate them."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      justifyContent: 'center',
      gap: 8,
    ...
    stroke: s <= stars ? '#F59E0B' : 'var(--pg-ink-300)',
    strokeWidth: "1.5"
  ...
  }, lang === 'pt' ? 'Depois' : lang === 'es' ? 'Después' : 'Later'), /*#__PURE__*/React.createElement("button", {
    onClick: () => onSubmit(target, stars, comment),
    disabled: stars === 0,
    style: {
      flex: 2,
      height: 48,
  ...
  }, lang === 'pt' ? 'Enviar avaliação' : lang === 'es' ? 'Enviar calificación' : 'Submit rating')), onDecline && /*#__PURE__*/React.createElement("button", {
    onClick: () => onDecline(target),
    style: {
      display: 'block',
      width: '100%',
      marginTop: 12,
...
function QuickPoolsScreen({
  ...
  const loadPendingOutRatings = React.useCallback(() => {
    ...
    window.sb.from('ratings').select('id,listing_name,to_id,connection_id').eq('from_id', user.uid).eq('connection_type', 'quickpool').or('stars.is.null').or('skipped_at.is.null').or('expires_at.is.null,expires_at.gte.' + new Date().toISOString()).then(({
      data
    }) => setPendingOutRatings(data || []));
  }, [user?.uid]);
  React.useEffect(() => {
  ...
  const loadJobs = React.useCallback(async () => {
    ...
    try {
      ...
      const {
        data
      } = await window.sb.from('quick_pool_jobs_feed').select('*').in('status', ['open', 'filled']).order('created_at', {
        ascending: false
      }).limit(50);
      {
  ...
  const formatRemaining = expiresAt => {
    ...
    if (h >= 24) {
      ...
      return lang === 'pt' ? `${d}d restantes` : lang === 'es' ? `${d}d restantes` : `${d}d left`;
    }
  ...
  React.useEffect(() => {
    ...
    window.sb.from('quick_pool_applications').select('job_id').eq('applicant_id', user.uid).neq('status', 'withdrawn').then(({
      data
    }) => {
      if (!data || data.length === 0) return;
      setApplied(prev => {
  ...
  React.useEffect(() => {
    ...
    if (!window.sb) return;
    window.sb.from('quick_pool_jobs_feed').select('*').eq('id', id).single().then(({
      data
    }) => {
      if (!data) return;
      openJobDetail({
  ...
  const deleteJob = async (jobId, e) => {
    ...
    const {
      data: accepted
    } = await window.sb.from('quick_pool_applications').select('id, applicant_id').eq('job_id', jobId).eq('status', 'accepted').limit(1);
    if (accepted && accepted[0]) {
      await window.sb.from('quick_pool_applications').update({
        status: 'cancelled'
      }).eq('id', accepted[0].id);
      window.sendPush && window.sendPush(accepted[0].applicant_id, lang === 'pt' ? '⚠️ Vaga cancelada' : lang === 'es' ? '⚠️ Vacante cancelada' : '⚠️ Job cancelled', lang === 'pt' ? 'O dono cancelou esta vaga depois de ter aceitado sua candidatura.' : lang === 'es' ? 'El propietario canceló esta vacante después de aceptar tu solicitud.' : 'The poster cancelled this job after accepting your application.', '/#express-pools', 'quick');
    }
  ...
  const extendJob = async (jobId, hours) => {
    ...
    const {
      data: verify
    } = await window.sb.from('quick_pool_jobs').select('expires_at').eq('id', jobId).single();
  ...
  const JobCard = ({
    ...
    }, "\uD83D\uDEA8 ", lang === 'pt' ? 'ROTA' : lang === 'es' ? 'RUTA' : 'ROUTE'), j.category === 'service' && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 10,
        fontWeight: 800,
        padding: '2px 8px',
    ...
    }, "\uD83D\uDD27 ", serviceTypeLabel(j.serviceType, lang)), !isAccepted && j.status === 'filled' && !isOwn && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 10,
        fontWeight: 700,
        padding: '2px 8px',
    ...
    }, Icon.briefcase(9, 'var(--pg-blue-700)'), qpApplicantCounts[j.id], " ", qpApplicantCounts[j.id] === 1 ? lang === 'pt' ? 'candidato' : lang === 'es' ? 'candidato' : 'applicant' : lang === 'pt' ? 'candidatos' : lang === 'es' ? 'candidatos' : 'applicants')), /*#__PURE__*/React.createElement("h3", {
      style: {
        margin: '0 0 6px',
    ...
    }, Icon.clock(14, 'var(--pg-ink-700)')) : /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 10.5,
        color: 'var(--pg-ink-400)',
        fontWeight: 600,
  ...
  const activateRoute = async route => {
    ...
    if (route.last_activated_job_id) {
      ...
      if (existing && (existing.status === 'open' || existing.status === 'filled')) {
        setRouteBusy(false);
        setRoutesOpen(false);
        showToast && showToast(existing.status === 'filled' ? lang === 'pt' ? 'Já tem alguém escalado nessa rota hoje' : lang === 'es' ? 'Ya hay alguien asignado a esta ruta hoy' : 'Someone is already assigned to this route today' : lang === 'pt' ? 'Essa rota já está ativa hoje' : lang === 'es' ? 'Esta ruta ya está activa hoy' : 'This route is already active today');
  ...
  if (isDesktop) {
    ...
    }, lang === 'pt' ? 'Assine Premium para ver quem publicou, candidatar-se e configurar suas cidades/dias.' : 'Subscribe to Premium to see who posted, apply, and set your cities/days.')), Icon.chev(14, '#6d28d9')), pendingOutRatings.length > 0 && /*#__PURE__*/React.createElement("button", {
      onClick: () => setRateNowTarget(pendingOutRatings[0]),
      style: {
        width: '100%',
        textAlign: 'left',
        border: '1px solid #FCD34D',
    ...
    }, pendingOutRatings.length > 1 ? lang === 'pt' ? `Você tem ${pendingOutRatings.length} avaliações pendentes` : `You have ${pendingOutRatings.length} pending ratings` : lang === 'pt' ? 'Você tem uma avaliação pendente' : 'You have a pending rating'), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11.5,
        color: '#B45309',
        marginTop: 2,
    ...
    }, pendingOutRatings[0].listing_name || (lang === 'pt' ? 'Toque para avaliar' : 'Tap to rate'))), Icon.chev(14, '#B45309')), sortedJobs.length === 0 ? /*#__PURE__*/React.createElement("div", {
      style: {
        padding: '48px 24px',
        textAlign: 'center',
        background: 'var(--pg-white)',
    ...
    }), rateNowTarget && /*#__PURE__*/React.createElement(PendingRatingModal, {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, lang === 'pt' ? 'Veja quem publicou, candidate-se e configure suas cidades/dias.' : 'See who posted, apply, and set your cities/days.')), Icon.chev(13, '#6d28d9'))), pendingOutRatings.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '10px 18px 0'
    }
  }, /*#__PURE__*/React.createElement("button", {
  ...
  }, pendingOutRatings[0].listing_name || (lang === 'pt' ? 'Toque para avaliar' : 'Tap to rate'))), Icon.chev(13, '#B45309'))), sortedJobs.length === 0 ? /*#__PURE__*/React.createElement("div", {
    style: {
      margin: '10px 18px 0',
      padding: '40px 20px',
      textAlign: 'center',
    ...
    }, pt ? 'Você negou a permissão antes. Siga os passos para reativar:' : 'You previously denied permission. Follow these steps to re-enable:'), isIOS && isStandalone && /*#__PURE__*/React.createElement(React.Fragment, null, step(1, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, "Abra o app ", /*#__PURE__*/React.createElement("b", null, "Ajustes"), " do iPhone") : /*#__PURE__*/React.createElement(React.Fragment, null, "Open iPhone ", /*#__PURE__*/React.createElement("b", null, "Settings"))), step(2, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, "Role e toque em ", /*#__PURE__*/React.createElement("b", null, "PoolGuyX")) : /*#__PURE__*/React.createElement(React.Fragment, null, "Scroll and tap ", /*#__PURE__*/React.createElement("b", null, "PoolGuyX"))), step(3, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, "Toque em ", /*#__PURE__*/React.createElement("b", null, "Notifica\xE7\xF5es"), " e ative o bot\xE3o") : /*#__PURE__*/React.createElement(React.Fragment, null, "Tap ", /*#__PURE__*/React.createElement("b", null, "Notifications"), " and toggle ON")), step(4, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("b", null, "Volte ao app"), " e toque em \"Ativar\" no banner") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("b", null, "Return to the app"), " and tap \"Enable\""))), isIOS && !isStandalone && /*#__PURE__*/React.createElement(React.Fragment, null, step(1, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, "No Safari, toque em ", /*#__PURE__*/React.createElement("b", null, "\u238B Compartilhar"), " (barra inferior)") : /*#__PURE__*/React.createElement(React.Fragment, null, "In Safari, tap ", /*#__PURE__*/React.createElement("b", null, "\u238B Share"), " (bottom bar)")), step(2, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, "Toque em ", /*#__PURE__*/React.createElement("b", null, "\"Adicionar \xE0 Tela de In\xEDcio\"")) : /*#__PURE__*/React.createElement(React.Fragment, null, "Tap ", /*#__PURE__*/React.createElement("b", null, "\"Add to Home Screen\""))), step(3, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, "Abra o app pela ", /*#__PURE__*/React.createElement("b", null, "tela inicial"), " do iPhone") : /*#__PURE__*/React.createElement(React.Fragment, null, "Open the app from your ", /*#__PURE__*/React.createElement("b", null, "Home Screen"))), step(4, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, "Toque em \"Ativar\" no banner de notifica\xE7\xF5es") : /*#__PURE__*/React.createElement(React.Fragment, null, "Tap \"Enable\" on the notification banner"))), !isIOS && /*#__PURE__*/React.createElement(React.Fragment, null, step(1, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, "Toque no \xEDcone de ", /*#__PURE__*/React.createElement("b", null, "cadeado"), " na barra de endere\xE7o") : /*#__PURE__*/React.createElement(React.Fragment, null, "Tap the ", /*#__PURE__*/React.createElement("b", null, "lock icon"), " in the address bar")), step(2, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, "Toque em ", /*#__PURE__*/React.createElement("b", null, "Permiss\xF5es \u2192 Notifica\xE7\xF5es \u2192 Permitir")) : /*#__PURE__*/React.createElement(React.Fragment, null, "Tap ", /*#__PURE__*/React.createElement("b", null, "Permissions \u2192 Notifications \u2192 Allow"))), step(3, pt ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("b", null, "Recarregue a p\xE1gina"), " e toque em \"Ativar\"") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("b", null, "Reload the page"), " and tap \"Enable\""))), /*#__PURE__*/React.createElement("button", {
      onClick: () => setNotifHelpOpen(false),
      style: {
        width: '100%',
        height: 50,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        fontWeight: 800,
        letterSpacing: '0.04em',
        color: isToday ? '#0D7280' : 'var(--pg-ink-700)'
      }
    }, (ROUTE_DAY_LABELS[day]?.[lang] || day).toUpperCase()), isToday && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 10,
        fontWeight: 800,
        padding: '2px 8px',
      ...
      }, route.name || cities.join(' · ') || (lang === 'pt' ? 'Rota sem nome' : lang === 'es' ? 'Ruta sin nombre' : 'Unnamed route')), route.name && /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 12.5,
          color: 'var(--pg-ink-600)',
          marginTop: 2
        }
      }, cities.join(' · ') || (lang === 'pt' ? 'Sem cidades' : 'No cities'))), /*#__PURE__*/React.createElement("button", {
...
function RoutePostForm({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, lang === 'pt' ? 'Piscinas desta rota' : lang === 'es' ? 'Piscinas de esta ruta' : 'Pools on this route'), /*#__PURE__*/React.createElement("button", {
    onClick: addPool,
    style: {
      width: 32,
      height: 32,
    ...
    const accessBadges = [p.dog && '🐕', p.saltwater && '🧂', p.poolType === 'condo' && p.gateCode && '🔑', p.poolType === 'condo' && p.doorman && '🚪'].filter(Boolean);
    return /*#__PURE__*/React.createElement("div", {
      key: i,
      style: {
        display: 'flex',
    ...
    }, p.city || (lang === 'pt' ? 'Sem cidade' : lang === 'es' ? 'Sin ciudad' : 'No city'), p.address ? ` · ${p.address}` : '', accessBadges.length > 0 ? ` · ${accessBadges.join(' ')}` : ''), /*#__PURE__*/React.createElement("div", {
      style: {
        marginLeft: 'auto',
        flexShrink: 0,
        display: 'flex',
      ...
      title: lang === 'pt' ? 'Duplicar' : lang === 'es' ? 'Duplicar' : 'Duplicate',
      style: {
        width: 28,
        height: 28,
        borderRadius: 8,
    ...
    }, Icon.chev(15, 'currentColor')))), isOpen && /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        flexDirection: 'column',
        gap: 14,
...
function QuickPoolDetails({
  ...
  const submitCompletionDispute = async (reasonId, description) => {
    ...
    try {
      if (window.sb && user?.uid && acceptedApp) {
        ...
        if (error && !dup) showToast && showToast('❌ ' + (error.message || 'Error'));else showToast && showToast(dup ? lang === 'pt' ? '⚠ Você já tem um problema reportado nessa vaga.' : lang === 'es' ? '⚠ Ya reportaste un problema en esta vacante.' : '⚠ You already have an open report on this job.' : lang === 'pt' ? '✅ Problema reportado.' : lang === 'es' ? '✅ Problema reportado.' : '✅ Problem reported.');
      }
    } catch (e) {
      showToast && showToast('❌ ' + (e?.message || 'Error'));
    } finally {
  ...
  const submitOwnerRatingAndFinishPoolGuy = async () => {
    ...
    if (window.sb) {
      ...
      await window.sb.from('ratings').insert({
        ...
        console.error('[QuickPools] rating placeholder insert failed', error);
      }); // ignore duplicate-key — placeholder may already exist
    }
  ...
  React.createElement("div", {
    ...
    onClick: () => setPhotoDisputeOpen(true),
    style: {
      height: 42,
      borderRadius: 12,
      border: '1.5px solid #FCA5A5',
    ...
    x2: "12.01",
    y2: "17"
  })), lang === 'pt' ? 'Reportar problema nas fotos' : lang === 'es' ? 'Reportar problema con las fotos' : 'Report a photo problem')) : job.status === 'filled' ? /*#__PURE__*/React.createElement("div", {
    style: {
      height: 50,
      borderRadius: 14,
