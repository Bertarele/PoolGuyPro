...
function LoginScreen({ onLogin, lang='en', setLang }) {
  ...
  const friendlyAuthError = (e) => {
    ...
    const L = (en, pt, es) => lang==='pt' ? pt : lang==='es' ? es : en;
    if (code === 'over_email_send_rate_limit' || /rate limit/i.test(raw) || (e && e.status === 429))
    ...
    if (/email not confirmed/i.test(raw))
      return L('Confirm your email first — check your inbox for the link.',
               'Confirme seu e-mail antes de entrar — veja o link na sua caixa de entrada.',
               'Confirma tu correo primero — revisa el enlace en tu bandeja de entrada.');
    if (/already registered|already been registered/i.test(raw))
      return L('This email already has an account. Try logging in.',
               'Este e-mail já tem conta. Tente entrar.',
               'Este correo ya tiene cuenta. Intenta iniciar sesión.');
    if (/password.*(short|least|weak)/i.test(raw))
      return L('Password is too weak — use at least 8 characters.',
  ...
  const handleForgot = async () => {
    ...
    const em = email.trim();
    if (!/^\S+@\S+\.\S+$/.test(em)) {
      setError(L('Type your email above first, then tap "Forgot password".',
                 'Digite seu e-mail acima e toque em "Esqueci a senha".',
                 'Escribe tu correo arriba y toca "Olvidé mi contraseña".'));
      return;
    }
    setLoading(true);
    ...
    setNotice(L('If that email has an account, we sent a link to reset your password.',
                'Se esse e-mail tiver conta, enviamos um link para redefinir a senha.',
                'Si ese correo tiene cuenta, enviamos un enlace para restablecer la contraseña.'));
  };
  ...
  const goSignup = () => { setMode('signup'); setStep(1); setNotice(''); setError(''); };
  const goLogin  = () => { setMode('login'); setStep(1); setNotice(''); setError(''); };
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      if (!data.session) {
        ...
        const insertProfile = () => window.sb.from('profiles').insert({ id: userId, name: name.trim(), region, role: 'user', email: email.trim() }).catch(()=>{});
        await insertProfile();
        setLoading(false);
  ...
  const FormContent = () => (<>
    {mode === 'login' ? (<>
      ...
      <div style={{textAlign:'right', marginTop:-4}}>
        <button onClick={handleForgot} disabled={loading} style={{border:'none', background:'transparent', color: isDesktop ? 'var(--pg-blue-500)' : '#ffffff',
  ...
  if (emailConfirmNeeded) {
    const confirmLabels = {
      en: { title: 'Check your email', sub: 'We sent a confirmation link to', action: 'Once confirmed, come back and log in.', btn: 'Back to login' },
      pt: { title: 'Verifique seu email', sub: 'Enviamos um link de confirmação para', action: 'Após confirmar, volte aqui e faça login.', btn: 'Voltar ao login' },
      es: { title: 'Revisa tu email', sub: 'Enviamos un enlace de confirmación a', action: 'Una vez confirmado, vuelve e inicia sesión.', btn: 'Volver al login' },
    };
    const cl = confirmLabels[lang] || confirmLabels.en;
    return (
      <div style={{width:'100%', minHeight:'100%', display:'flex', alignItems:'center', justifyContent:'center', background:'linear-gradient(160deg,#0B1F32 0%,#040D18 100%)', padding:24}}>
        <div style={{maxWidth:360, width:'100%', textAlign:'center'}}>
          <div style={{width:72, height:72, borderRadius:'50%', background:'rgba(0,119,182,0.15)', border:'1px solid rgba(0,119,182,0.35)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 20px', fontSize:32}}>
            ✉️
          </div>
          <div style={{fontSize:22, fontWeight:800, color:'#e8edf2', marginBottom:10}}>{cl.title}</div>
          <div style={{fontSize:14, color:'rgba(255,255,255,0.5)', lineHeight:1.65, marginBottom:6}}>{cl.sub}</div>
          <div style={{fontSize:14, fontWeight:700, color:'#60c5f0', marginBottom:16, wordBreak:'break-all'}}>{email}</div>
          <div style={{fontSize:13, color:'rgba(255,255,255,0.4)', lineHeight:1.65, marginBottom:28}}>{cl.action}</div>
          <button onClick={()=>{setEmailConfirmNeeded(false); setMode('login'); setPass(''); setPassConfirm('');}}
            style={{width:'100%', height:48, borderRadius:12, background:'linear-gradient(135deg,#0c4a6e,#0077B6)', border:'none',
            ...
            {mode === 'login' && (<>
              ...
              <div style={{display:'flex', flexDirection:'column', gap:11}}>
                ...
                <div style={{textAlign:'right', marginTop:-8}}>
                  <button onClick={handleForgot} disabled={loading} style={{border:'none', background:'transparent', color:'#1565E8', fontSize:12, fontWeight:600, cursor:'pointer', padding:0, fontFamily:'inherit'}}>{t.forgotPw}</button>
                </div>
                {notice && <div style={{fontSize:12.5, color:'#166534', background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:10, padding:'9px 12px', fontWeight:500}}>{notice}</div>}
                {error && <div style={{fontSize:12.5, color:'#ef4444', background:'#fef2f2', border:'1px solid #fecaca', borderRadius:10, padding:'9px 12px', fontWeight:500}}>{error}</div>}
                <button onClick={handleLogin} disabled={!canSubmit||loading} style={{
                ...
                <button onClick={()=>{const b='https://xiszfqghizqzlwyrfjol.supabase.co';const r=window.location.origin||'https://poolguyx.com';window.location.href=b+'/auth/v1/authorize?provider=google&prompt=select_account&redirect_to='+encodeURIComponent(r);}}
                  style={{width:'100%', height:50, borderRadius:12, border:'1.5px solid #e2e8f0', background:'#fff', cursor:'pointer', fontFamily:'inherit', fontWeight:600, fontSize:14, color:'#3c4043', display:'flex', alignItems:'center', justifyContent:'center', gap:10}}>
      ...
      <div style={{
        ...
        {mode === 'login' && (<>
          ...
          <div style={{textAlign:'right', marginTop:-6}}>
            <button onClick={handleForgot} disabled={loading} style={{border:'none', background:'transparent', color:'#0A2840',
          ...
          <div style={{display:'flex', flexDirection:'column', gap:10}}>
            <button onClick={() => {
              ...
              background:'#fff', boxShadow:'0 2px 10px rgba(0,0,0,0.18)', cursor:'pointer', fontFamily:'inherit', fontWeight:600, fontSize:14,
              color:'#3c4043', display:'flex', alignItems:'center', justifyContent:'center', gap:10}}>
            ...
            <button onClick={() => {
              ...
              background:'#000', boxShadow:'0 2px 10px rgba(0,0,0,0.25)', cursor:'pointer', fontFamily:'inherit', fontWeight:600, fontSize:14,
...
function RecoveryGate({ lang = 'en' }) {
  ...
  const save = async () => {
    ...
    setBusy(false);
    if (error) {
      setErr(/same|different/i.test(error.message || '')
        ? L('Choose a password different from the old one.', 'Escolha uma senha diferente da anterior.', 'Elige una contraseña diferente a la anterior.')
        : (error.message || L('Could not save. Try again.', 'Não foi possível salvar. Tente de novo.', 'No se pudo guardar. Inténtalo de nuevo.')));
      return;
    }
  ...
  const field = { width:'100%', height:48, borderRadius:12, border:'1px solid #cfd8e3', padding:'0 14px', fontSize:15, fontFamily:'inherit', boxSizing:'border-box', color:'#0A2840', background:'#fff' };
  return (
    <div style={{position:'fixed', inset:0, zIndex:100000, background:'rgba(4,13,24,0.72)', display:'flex', alignItems:'center', justifyContent:'center', padding:20}}>
      <div role="dialog" aria-modal="true" style={{width:'100%', maxWidth:400, background:'#fff', borderRadius:20, padding:'26px 22px', boxShadow:'0 24px 70px rgba(0,0,0,0.4)', display:'flex', flexDirection:'column', gap:12}}>
        {done ? (
          <div style={{textAlign:'center', padding:'14px 0', color:'#166534', fontWeight:700, fontSize:16}}>
            ✓ {L('Password updated', 'Senha atualizada', 'Contraseña actualizada')}
          </div>
        ) : (<>
          <div style={{fontSize:19, fontWeight:800, color:'#0A2840'}}>{L('Choose a new password', 'Escolha uma nova senha', 'Elige una nueva contraseña')}</div>
          <div style={{fontSize:13, color:'#5b7186', lineHeight:1.45}}>
            {L('At least 8 characters, with upper case, lower case and a number.',
               'Pelo menos 8 caracteres, com maiúscula, minúscula e número.',
               'Al menos 8 caracteres, con mayúscula, minúscula y número.')}
          </div>
          <input type="password" autoComplete="new-password" value={pass} onChange={e=>setPass(e.target.value)} style={field}
            placeholder={L('New password', 'Nova senha', 'Nueva contraseña')}/>
          <input type="password" autoComplete="new-password" value={pass2} onChange={e=>setPass2(e.target.value)} style={field}
            onKeyDown={e=>e.key==='Enter'&&save()} placeholder={L('Confirm new password', 'Confirmar nova senha', 'Confirmar nueva contraseña')}/>
          {pass2 && pass !== pass2 && <div style={{fontSize:12.5, color:'#b91c1c'}}>{L('Passwords do not match.', 'As senhas não coincidem.', 'Las contraseñas no coinciden.')}</div>}
          {err && <div style={{fontSize:12.5, color:'#b91c1c', background:'#fef2f2', border:'1px solid #fecaca', borderRadius:10, padding:'9px 12px'}}>{err}</div>}
          <button onClick={save} disabled={!strong || pass !== pass2 || busy} style={{height:50, borderRadius:12, border:'none', fontFamily:'inherit', fontSize:15, fontWeight:700, color:'#fff', cursor:(strong && pass===pass2)?'pointer':'default',
            background:'linear-gradient(90deg,#1565E8 0%,#00C2D4 100%)', opacity:(strong && pass===pass2 && !busy)?1:0.45}}>
            {busy ? '…' : L('Save password', 'Salvar senha', 'Guardar contraseña')}
          </button>
          <button onClick={()=>{ window._pgRecovery=false; setOpen(false); }} style={{border:'none', background:'transparent', color:'#5b7186', fontSize:13, cursor:'pointer', fontFamily:'inherit', padding:4}}>
