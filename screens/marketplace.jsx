...
function MapChooserSheet({ address, lang, onClose }) {
  const isIOS = typeof navigator !== 'undefined' && /iP(hone|od|ad)/.test(navigator.userAgent);
  const q = encodeURIComponent(address);
  const options = [
    isIOS && { key:'apple', label:'Apple Maps', href:`https://maps.apple.com/?q=${q}`, color:'#0EBAC7' },
    { key:'google', label:'Google Maps', href:`https://www.google.com/maps/search/?api=1&query=${q}`, color:'#0EBAC7' },
    { key:'waze', label:'Waze', href:`https://waze.com/ul?q=${q}&navigate=yes`, color:'#0EBAC7' },
  ].filter(Boolean);
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        width:'100%', maxWidth:480, background:'var(--pg-white)', borderRadius:'20px 20px 0 0',
        padding:'8px 0 max(12px, env(safe-area-inset-bottom)) 0', boxShadow:'0 -8px 30px rgba(0,0,0,0.25)',
      }}>
        <div style={{width:36,height:4,borderRadius:2,background:'var(--pg-ink-200)',margin:'6px auto 12px'}}/>
        <div style={{padding:'0 18px 12px',fontSize:12,fontWeight:700,color:'var(--pg-ink-500)',textTransform:'uppercase',letterSpacing:'.06em'}}>
          {lang==='pt'?'Abrir endereço em':lang==='es'?'Abrir dirección en':'Open address in'}
        </div>
        {options.map(opt => (
          <a key={opt.key} href={opt.href} target="_blank" rel="noopener noreferrer" onClick={onClose}
            style={{display:'flex',alignItems:'center',gap:12,padding:'13px 18px',textDecoration:'none',
              borderTop:'1px solid var(--pg-ink-100)'}}>
            <div style={{width:34,height:34,borderRadius:'50%',background:'rgba(14,186,199,0.12)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
              {Icon.pin(15,opt.color)}
            </div>
            <span style={{fontSize:14,fontWeight:700,color:'var(--pg-ink-900)'}}>{opt.label}</span>
          </a>
        ))}
        <button onClick={onClose} style={{
          width:'calc(100% - 32px)', margin:'12px 16px 0', padding:'13px', borderRadius:12,
...
function ViewListingSheet({ item, lang, onClose, openChat, openPublicProfile, goTab, openEditProfile, isAdmin, canDelete, onEdit, currentUser, showToast, onDeleted, isSaved, onToggleSave, onShare, liveMarket=[], onOpenListing, onAfterSold }) {
  ...
  React.useEffect(() => {
    ...
    window.sb.from('rental_requests')
      .select('id, status, requester_id, requester_name, created_at, period, quantity, total_price, requester_verified, owner_kept_active, meetup_address, start_date, end_date, owner_rated, renter_rated')
      .eq('listing_id', item._id)
      .then(({ data }) => {
        if (!data) return;
  ...
  React.useEffect(() => {
    ...
    const check = () => window.sb.from('rental_requests')
      .select('id, status, requester_id, meetup_address, end_date')
      .eq('listing_id', item._id)
      .then(({ data }) => {
  ...
  const handleSaveMeetupAddress = async (requestId, req) => {
    ...
    if (error) { showToast && showToast('❌ ' + (error.message||'Error')); return; }
    setOwnerRequests(prev => prev.map(r => r.id === requestId ? {...r, meetup_address: addr || null} : r));
    setMeetupEditingId(null);
    showToast && showToast(lang==='pt'?'✓ Endereço salvo!':'✓ Address saved!');
    if (addr && req?.requester_id) {
      _notify(req.requester_id, 'rental_approved',
        { en:'Meetup location added', pt:'Local de encontro adicionado', es:'Ubicación de encuentro agregada' },
        { en:`The owner added a meetup address for "${item.name||''}"`, pt:`O dono adicionou um endereço de encontro para "${item.name||''}"`, es:`El propietario agregó una dirección de encuentro para "${item.name||''}"` },
        item._id);
    }
  };
  ...
  const handleSubmitRating = async () => {
    ...
    if (ratingSheet.requestId) {
      window.sb.from('rental_requests')
        .update(amOwner ? { owner_rated: true } : { renter_rated: true })
    ...
    const rateBodyObj = { en:`${rateMyName} rated you! Rate them back.`, pt:`${rateMyName} avaliou você! Avalie-o também.`, es:`${rateMyName} te calificó! Califícalo también.` };
    window.sb.from('notifications').insert({
      user_id: ratingSheet.rateeId, type: 'rating',
  ...
  const RequestRentalBlock = () => {
    ...
    if (reqStatus === 'approved') {
      ...
      <div style={{borderRadius:14,overflow:'hidden',border:'1.5px solid rgba(14,186,199,0.40)'}}>
        ...
        {myMeetupAddress && (
          <button onClick={()=>setMapChooserAddr(myMeetupAddress)}
            style={{display:'flex',alignItems:'center',gap:10,padding:'11px 16px',background:'var(--pg-white)',
              border:'none',borderTop:'1px solid rgba(14,186,199,0.20)',
              width:'100%',cursor:'pointer',fontFamily:'inherit',textAlign:'left'}}>
            <div style={{width:30,height:30,borderRadius:'50%',background:'var(--pg-ink-100)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
              {Icon.pin(14,'#0EBAC7')}
            </div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:11,fontWeight:700,color:'var(--pg-ink-500)'}}>{lang==='pt'?'Local de encontro':'Meetup location'}</div>
              <div style={{fontSize:12.5,fontWeight:600,color:'var(--pg-ink-800)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{myMeetupAddress}</div>
            </div>
            <span style={{fontSize:11,fontWeight:700,color:'#0EBAC7',flexShrink:0,display:'flex',alignItems:'center',gap:3}}>
              {lang==='pt'?'Abrir mapa':'Open map'}
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#0EBAC7" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6"/></svg>
            </span>
          </button>
        )}
        ...
        {ownerRequests.map(req => {
          ...
          return (
            <div key={req.id} style={{
              ...
              {isAppr && (
                <>
                  {/* Meetup address — the renter sees this as a tap-to-open-in-maps link */}
                  <div style={{marginTop:9,borderRadius:12,overflow:'hidden',border:'1.5px solid var(--pg-ink-200)'}}>
                    <div style={{padding:'9px 11px',background:'var(--pg-ink-50)'}}>
                      <div style={{fontSize:11.5,fontWeight:800,color:'var(--pg-ink-700)',display:'flex',alignItems:'center',gap:5}}>
                        {Icon.pin(12,'var(--pg-ink-500)')}
                        {lang==='pt'?'Local de encontro':'Meetup location'}
                      </div>
                      {meetupEditingId === req.id ? (
                        ...
                        <div style={{marginTop:6,display:'flex',alignItems:'center',gap:8}}>
                          <div style={{flex:1,fontSize:12.5,color: req.meetup_address ? 'var(--pg-ink-700)' : 'var(--pg-ink-400)',
  ...
  if (isDesktop) {
    return (
      <div style={{minHeight:'100vh', background:'var(--pg-ink-50)'}}>
        ...
        {mapChooserAddr && (
          <MapChooserSheet address={mapChooserAddr} lang={lang} onClose={()=>setMapChooserAddr(null)}/>
        )}
        {MarkSoldSheetSlot()}
        {RatingOverlay()}
        {DisputeFormSheet()}
  ...
  const renderLiveCard = (item, desktopMode=false) => {
    ...
    return (
      <button key={item._id}
        ...
        style={{
          padding:0, overflow:'hidden', position:'relative', cursor: (isSoldItem || isActiveRental) ? (isMyPost(item) ? 'pointer' : 'default') : 'pointer',
          border: (desktopMode && darkMode)
            ? `1.5px solid ${(isSoldItem || isActiveRental) ? '#30363D' : '#21262D'}`
            : isPending ? '1.5px solid var(--pg-ink-200)' : (isSoldItem || isActiveRental) ? '1.5px solid var(--pg-ink-200)' : '1.5px solid var(--pg-blue-100)',
          display:'flex', flexDirection:'column',
          borderRadius:16,
          background: (desktopMode && darkMode)
            ? ((isSoldItem || isActiveRental) ? '#161B22' : '#1C2128')
            : ((isSoldItem || isActiveRental) ? 'var(--pg-ink-50)' : 'var(--pg-white)'),
        ...
        <div style={{opacity: isPending ? 0.82 : isSoldItem ? 0.70 : isActiveRental ? 0.75 : 1, filter: (isSoldItem || isActiveRental) ? 'grayscale(0.65)' : 'none', display:'flex', flexDirection:'column', flex:1}}>
        {/* Photo */}
              ...
              .map(item => {
                ...
                return (
                  <button key={item._id}
                    ...
                    <div style={{opacity: isPending ? 0.82 : isSoldItem ? 0.65 : isActiveRental ? 0.72 : 1, filter: (isSoldItem || isActiveRental) ? 'grayscale(0.6)' : 'none', display:'flex', flexDirection:'column', flex:1}}>
                          ...
                          <span style={{fontFamily:'var(--pg-font-display)', fontSize:22, fontWeight:800,
                            ...
                            {(() => {
                              ...
                              if (item.rentPrices && typeof item.rentPrices==='object') {
                                ...
                                return <>{entries.length>1&&<span style={{fontSize:10,fontWeight:600,color:'var(--pg-ink-400)',marginRight:2}}>{lang==='en'?'from':'de'}</span>}${fmtN(first.v, lang)}<span style={{fontSize:11,fontWeight:500,color:'var(--pg-ink-400)',marginLeft:2}}>{sfx}</span></>;
                              }
