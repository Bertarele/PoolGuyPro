...
function ChatConversation({ convo, lang, t, onBack, onClose, currentUser, onUnreadChange, onOpenListing, openPublicProfile }) {
  ...
  const handleVacDecision = async (decision) => {
    ...
    if (status === 'accepted' && selectedDays.length) {
      ...
      const { data: bdRows, error: bdErr } = await window.sb.from('vacations')
        .update({ booked_days: merged }).eq('id', rawId).select('id');
...
function ApplicantsSheet({ open, onClose, post, lang='en', onChat, user, onOpenProfile, openRating, showToast }) {
    ...
    try {
      if (window.sb && user?.uid) {
        ...
        if (error && !dup) showToast && showToast('❌ ' + (error.message || 'Error'));
          ...
          ? (lang==='pt' ? '⚠ Você já tem uma reclamação aberta sobre esta cobertura.' : lang==='es' ? '⚠ Ya tienes un reclamo abierto sobre esta cobertura.' : '⚠ You already have an open report on this coverage.')
  ...
  const closeJobAndRejectOthers = async (acceptedId) => {
    ...
    await window.sb.from('jobs').update({ hired_at: new Date().toISOString() }).eq('id', post._id);
    const others = applicants.filter(a => a.id !== acceptedId && a.status !== 'rejected' && a._dbId);
    for (const other of others) {
      await dbUpdate(other._dbId, { status: 'rejected' });
      if (other.applicant_id) {
        window.sb.from('notifications').insert({
          user_id: other.applicant_id,
          type:    'job_rejected',
          title:   JSON.stringify({ en:'Application not selected', pt:'Candidatura não selecionada', es:'Postulación no seleccionada' }),
          body:    JSON.stringify({ en:`Your application for "${post.role||post.company||''}" was not selected — another candidate was hired.`, pt:`Sua candidatura para "${post.role||post.company||''}" não foi selecionada — outro candidato foi contratado.`, es:`Tu postulación para "${post.role||post.company||''}" no fue seleccionada — se contrató a otro candidato.` }),
          link_id: post._id || null,
          read:    false,
        });
      }
    }
    setApplicants(prev => prev.map(a => a.id===acceptedId ? a : (a.status!=='rejected' ? {...a, status:'rejected'} : a)));
  };
    ...
    if (post?._live && app?._dbId) {
      ...
      if (app.applicant_id && window.sb) {
        ...
        window.sendPush && window.sendPush(
          app.applicant_id,
          lang==='pt' ? '🎉 Candidatura aceita!' : lang==='es' ? '🎉 ¡Postulación aceptada!' : '🎉 Application accepted!',
          lang==='pt' ? `Sua candidatura para "${jobRole}" foi aceita.`
            : lang==='es' ? `Tu postulación para "${jobRole}" fue aceptada.`
            : `Your application for "${jobRole}" was accepted.`,
          '/#work', 'work'
        );
      }
    ...
    if (post?._live && app?._dbId) {
      ...
      if (app.applicant_id && window.sb && (status === 'accepted' || status === 'rejected')) {
        ...
        window.sendPush && window.sendPush(
          ...
          isAcc
            ? (lang==='pt' ? '🎉 Candidatura aceita!' : lang==='es' ? '🎉 ¡Postulación aceptada!' : '🎉 Application accepted!')
            : (lang==='pt' ? 'Candidatura não selecionada' : lang==='es' ? 'Postulación no seleccionada' : 'Application not selected'),
          isAcc
            ? (lang==='pt' ? `Sua candidatura para "${jobRole}" foi aceita.` : lang==='es' ? `Tu postulación para "${jobRole}" fue aceptada.` : `Your application for "${jobRole}" was accepted.`)
            : (lang==='pt' ? `Sua candidatura para "${jobRole}" não foi selecionada.` : lang==='es' ? `Tu postulación para "${jobRole}" no fue seleccionada.` : `Your application for "${jobRole}" was not selected.`),
          '/#work', 'work'
        );
      }
      ...
      if (status === 'accepted' && post.type === 'vacation' && app.selectedDays?.length) {
        ...
        const { data: bdRows, error: bdErr } = await window.sb.from('vacations')
          .update({ booked_days: merged }).eq('id', post._id).select('id');
        if (bdErr || !bdRows || bdRows.length === 0) {
  ...
  return (
    <Sheet open={open} onClose={onClose} height="88%">
            ...
            return (
              <div key={a.id} className="pg-card" style={{padding:'13px 14px'}}>
                ...
                {post.type === 'vacation' && a.status === 'accepted' && a.poolGuyDone && (
                  <div style={{marginTop:10, padding:'10px 12px', borderRadius:10, background:'oklch(0.96 0.04 260)', border:'0.5px solid oklch(0.85 0.08 260)'}}>
                    ...
                    {a.submittedPhotos && a.submittedPhotos.length > 0 && (() => {
                      const typeLabel = (t) => t ? (t.startsWith('custom:') ? t.slice(7)
                        : t==='before'   ? (lang==='pt'?'Antes':'Before')
                        : t==='after'    ? (lang==='pt'?'Depois':lang==='es'?'Después':'After')
                        : t==='vacuum'   ? (lang==='pt'?'Vacum':'Vacuum')
                        : t==='chemical' ? (lang==='pt'?'Químico':lang==='es'?'Químico':'Chemical') : t) : null;
                      ...
                      return Object.entries(groups).map(([label, photos]) => (
                        <div key={label} style={{marginBottom:10}}>
                          {label && (
                            <div style={{fontSize:10.5, fontWeight:700, color:'oklch(0.42 0.16 260)', marginBottom:5, display:'flex', alignItems:'center', gap:4}}>
                              {Icon.pin(10,'oklch(0.42 0.16 260)')} {label}
                            </div>
                          )}
                          <div style={{display:'flex', gap:8, flexWrap:'wrap'}}>
                            {photos.map((p,i) => (
                              <div key={i} style={{display:'flex', flexDirection:'column', alignItems:'center', gap:3}}>
                                ...
                                {typeLabel(p.type) && <span style={{fontSize:9.5, color:'oklch(0.48 0.14 260)', fontWeight:600}}>{typeLabel(p.type)}</span>}
                              </div>
                            ))}
                          </div>
                        </div>
                    ...
                    <button onClick={()=>setCompletionGateFor(a)} className="pg-btn pg-btn-primary" style={{width:'100%', height:38, fontSize:13, borderRadius:999}}>
                ...
                <div style={{display:'flex', gap:7, marginTop:11, flexWrap:'wrap'}}>
                  <button onClick={()=>{ onChat(a.applicant_id ? { id: a.applicant_id, name: a.name, listingId: post?._id ? (post.type==='vacation' ? 'vac_'+post._id : post.type==='quickpool' ? 'qp_'+post._id : 'job_'+post._id) : null, listingContext: { name: tr(post?.title, lang) || post?.role || post?.company || (post?.type==='vacation' ? (lang==='pt'?'Cobertura de férias':lang==='es'?'Cobertura de vacaciones':'Vacation coverage') : (lang==='pt'?'Vaga':'Job')), type: post?.type || 'hiring' } } : a.name); onClose(); }}
                      ...
                      <button disabled={a.status==='accepted' && !rejectReason.trim()} onClick={async ()=>{
                        ...
                        if (post?._live && a._dbId && window.sb) {
                          ...
                          if (wasAccepted && post.type === 'vacation' && a.selectedDays?.length) {
                            ...
                            const { data: bdRows2, error: bdErr2 } = await window.sb.from('vacations')
                              .update({ booked_days: remaining }).eq('id', post._id).select('id');
      ...
      {/* Owner checkpoint: "did the pool guy actually finish?" before finalizing vacation coverage */}
      <YesNoLaterGate
        ...
        lang={lang}
        onYes={()=>{ const a=completionGateFor; setCompletionGateFor(null); finalizeVacation(a); }}
        onLater={()=>{ const a=completionGateFor; setCompletionGateFor(null); finalizeVacation(a); }}
      ...
      <PhotoDisputeModal
        ...
        onClose={()=>{ const a=completionReportFor; setCompletionReportFor(null); if (a) finalizeVacation(a); }}
        lang={lang}
      />
    </Sheet>
  );
        ...
        {isAccepted && (
          <button
            onClick={async () => {
              ...
              onClose();
              setTimeout(() => {
                onChat(authorId ? { id: authorId, name: authorName, listingId: display.job_id ? 'job_'+display.job_id : null, listingContext: { name: tr(display.title, lang) || display.company || (lang==='pt'?'Vaga':'Job'), type: 'hiring' } } : null);
              }, 300);
            }}
      ...
      >
        ...
        <div style={{display:'flex', flexDirection:'column', alignItems:'center', marginBottom:20}}>
          ...
          <div style={{fontSize:12.5, color:'var(--pg-ink-500)', textAlign:'center', lineHeight:1.5}}>
            {lang==='pt'
              ? (listingName ? `Transação: "${listingName}"` : 'Avalie para revelar a nota deles')
              : lang==='es' ? (listingName ? `Transacción: "${listingName}"` : 'Evalúa para revelar su calificación')
              : (listingName ? `Transaction: "${listingName}"` : 'Rate them to reveal their rating')}
            {count > 1 && (
