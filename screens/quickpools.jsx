...
function QuickPoolsScreen({ ctx }) {
  ...
  React.useEffect(() => {
    if (!user?.uid) return;
    let seen = [];
    try { seen = JSON.parse(localStorage.getItem('pg_qp_notif_prompted') || '[]'); } catch(e) {}
    if (seen.includes(user.uid)) return;
    try { localStorage.setItem('pg_qp_notif_prompted', JSON.stringify([...seen, user.uid])); } catch(e) {}
  ...
  React.useEffect(() => {
    const handler = (e) => {
      ...
      if (jobId) setApplied(prev => ({ ...prev, [jobId]: true }));
    };
  ...
  const loadMyRoutes = React.useCallback(async () => {
    ...
    const { data } = await window.sb.from('quick_routes').select('*')
      .eq('owner_id', user.uid).eq('status', 'active').order('day_of_week');
    setMyRoutes(data || []);
    setRoutesLoading(false);
  }, [user?.uid]);
  React.useEffect(() => { if (routesOpen) loadMyRoutes(); }, [routesOpen, loadMyRoutes]);
  ...
  const saveRoute = async (data) => {
    ...
    const { error } = isEdit
      ? await window.sb.from('quick_routes').update(row).eq('id', data.id)
      : await window.sb.from('quick_routes').insert(row);
    setRouteBusy(false);
    if (error) { showToast && showToast('❌ ' + (error.message || 'Error')); return; }
    showToast && showToast(isEdit
      ? (lang==='pt'?'✓ Rota atualizada':lang==='es'?'✓ Ruta actualizada':'✓ Route updated')
      : (lang==='pt'?'✓ Rota cadastrada':lang==='es'?'✓ Ruta registrada':'✓ Route saved'));
    setRouteForm(null);
    loadMyRoutes();
  };
  ...
  const deleteRoute = async (route) => {
    ...
    setRouteBusy(true);
    const { error } = await window.sb.from('quick_routes').delete().eq('id', route.id);
    setRouteBusy(false);
    if (error) { showToast && showToast('❌ ' + (error.message || 'Error')); return; }
  ...
  const activateRoute = async (route) => {
    ...
    try {
      ...
      await window.sb.from('quick_routes').update({
        last_activated_job_id: inserted.id, last_activated_at: new Date().toISOString(),
      }).eq('id', route.id);
  ...
  const routesOverlay = (
    <>
      <FullPage open={routesOpen} onClose={()=>setRoutesOpen(false)}>
        <RouteManagerPanel
          lang={lang} routes={myRoutes} loading={routesLoading} busy={routeBusy}
      ...
      <FullPage open={!!routeForm} onClose={()=>setRouteForm(null)}>
        {routeForm && (
          <RoutePostForm lang={lang} initialValues={routeForm.id ? routeForm : null}
            onClose={()=>setRouteForm(null)} onSubmit={saveRoute}/>
        )}
      </FullPage>
    </>
  );
          ...
          return (
            <div style={{background:_bg, padding:'28px 40px 28px', position:'relative', overflow:'hidden'}}>
              ...
              <div style={{position:'absolute', inset:0, overflow:'hidden', pointerEvents:'none', zIndex:0}}>
                <img src="icone-watermark.webp" alt="" style={{position:'absolute', left:'50%', top:'63%', transform:'translate(-50%,-50%)', height:235, objectFit:'contain', opacity:0.60, userSelect:'none'}}/>
              </div>
          ...
          <NavyBar
            ...
            title={
              <div style={{display:'flex', alignItems:'center', gap:10}}>
                <div style={{width:44, height:44, borderRadius:13, flexShrink:0, background:H.iconBg, border:`0.5px solid ${H.border}`, display:'flex', alignItems:'center', justifyContent:'center', transform:'translateY(7px)'}}>
                  {poolIcon}
                </div>
                <div style={{paddingTop:2}}>
                  <div style={{fontSize:10.5, fontWeight:600, color:H.sub, letterSpacing:'0.10em', marginBottom:3, marginTop:-6, textTransform:'uppercase', whiteSpace:'nowrap', transform:'translateY(3px)'}}>
...
const ROUTE_DAY_LABELS = {
  mon:{pt:'Segunda',es:'Lunes',en:'Monday'}, tue:{pt:'Terça',es:'Martes',en:'Tuesday'},
  wed:{pt:'Quarta',es:'Miércoles',en:'Wednesday'}, thu:{pt:'Quinta',es:'Jueves',en:'Thursday'},
  fri:{pt:'Sexta',es:'Viernes',en:'Friday'}, sat:{pt:'Sábado',es:'Sábado',en:'Saturday'},
  sun:{pt:'Domingo',es:'Domingo',en:'Sunday'},
};
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', height:'100%'}}>
      <div style={{padding:'14px 18px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'0.5px solid var(--pg-ink-200)', flexShrink:0}}>
        ...
        <button onClick={onCreate} style={{border:'none', background:'transparent', color:'var(--pg-blue-500)', fontSize:15, fontWeight:700, cursor:'pointer', padding:0}}>
          {lang==='pt'?'+ Nova':lang==='es'?'+ Nueva':'+ New'}
        </button>
      </div>
      ...
      <div style={{flex:1, overflow:'auto', padding:'16px 18px'}}>
        ...
        {loading ? (
          <div style={{textAlign:'center', padding:'40px 0', color:'var(--pg-ink-400)', fontSize:13}}>
            {lang==='pt'?'Carregando…':lang==='es'?'Cargando…':'Loading…'}
          </div>
        ) : sorted.length === 0 ? (
          <div style={{textAlign:'center', padding:'40px 16px', color:'var(--pg-ink-400)', fontSize:13.5}}>
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', height:'100%'}}>
      <div style={{padding:'14px 18px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'0.5px solid var(--pg-ink-200)', flexShrink:0}}>
        ...
        <h2 style={{margin:0, fontFamily:'var(--pg-font-display)', fontSize:17, fontWeight:700, letterSpacing:'-0.01em'}}>
          {isEdit ? (lang==='pt'?'Editar rota':lang==='es'?'Editar ruta':'Edit route') : (lang==='pt'?'Nova rota':lang==='es'?'Nueva ruta':'New route')}
        </h2>
        <div style={{width:60}}/>
      </div>
      ...
      <div style={{flex:1, overflow:'auto', touchAction:'pan-y', padding:'20px 18px', display:'flex', flexDirection:'column', gap:20}}>
        ...
        <HiringFormSection label={lang==='pt'?'Dia da semana':lang==='es'?'Día de la semana':'Day of week'}>
          <div style={{display:'flex', gap:6, flexWrap:'wrap'}}>
            {dayDefs.map(d => {
              const on = dayOfWeek === d.id;
              return (
                <button key={d.id} onClick={()=>setDayOfWeek(d.id)} style={{
        ...
        <HiringFormSection label={lang==='pt'?'Nome da rota (opcional)':lang==='es'?'Nombre de la ruta (opcional)':'Route name (optional)'}>
          <input className="pg-field" value={name} onChange={e=>setName(e.target.value)}
...
function LeafletMapBlock({ jobs, highlighted, onPinClick, fullHeight=false }) {
  ...
  React.useEffect(() => {
    ...
    const attrCtl = L.control.attribution({ position:'bottomleft', prefix:false })
  ...
  return (
    ...
    <div style={{display:'flex', flexDirection:'column', minHeight:'100%'}}>
      ...
      <div style={{padding:'16px 18px 100px', flex:1}}>
        ...
        {/* Rota Rápida — per-pool breakdown (city/address/type/extras per pool).
            Address only ever appears here once accepted or for the poster —
            everything else about the pool is fine to show while browsing. */}
        {job.routePools && job.routePools.length > 0 && (() => {
          ...
          return (
            <div style={{marginTop:14, display:'flex', flexDirection:'column', gap:10}}>
              <div style={{fontSize:11, color:'var(--pg-ink-500)', fontWeight:600, letterSpacing:'0.05em'}}>
                {lang==='pt'?'PISCINAS DESTA ROTA':lang==='es'?'PISCINAS DE ESTA RUTA':'POOLS ON THIS ROUTE'}
              </div>
              {job.routePools.map((p, i) => (
                <div key={i} className="pg-card" style={{padding:'12px 14px'}}>
                  <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8}}>
                    <div style={{fontSize:13.5, fontWeight:800, color:'var(--pg-ink-900)'}}>
                      {(lang==='pt'?'Piscina':lang==='es'?'Piscina':'Pool')} {i+1} · {p.city}
                    </div>
                    <span style={{fontSize:10.5, fontWeight:700, padding:'2px 8px', borderRadius:999, background:'var(--pg-ink-100)', color:'var(--pg-ink-600)'}}>
                      {p.poolType==='condo'?(lang==='pt'?'Condomínio':'Condo'):(lang==='pt'?'Casa':'House')}
                    </span>
                  </div>
                  {canSeeAddress && p.address && (
                    <div style={{display:'flex', alignItems:'center', gap:6, fontSize:12.5, color:'var(--pg-blue-700)', fontWeight:600, marginBottom:8}}>
                      {Icon.pin(12,'var(--pg-blue-700)')} {p.address}
                    </div>
                  )}
                  <div style={{display:'flex', flexWrap:'wrap', gap:6}}>
                    {p.dog && <span style={{fontSize:11, fontWeight:700, padding:'3px 9px', borderRadius:999, background:'var(--pg-ink-100)', color:'var(--pg-ink-700)'}}>🐕 {lang==='pt'?'Cachorro':'Dog'}</span>}
                    {p.saltwater && <span style={{fontSize:11, fontWeight:700, padding:'3px 9px', borderRadius:999, background:'var(--pg-ink-100)', color:'var(--pg-ink-700)'}}>🧂 {lang==='pt'?'Sal':'Salt'}</span>}
                    {p.gateCode && <span style={{fontSize:11, fontWeight:700, padding:'3px 9px', borderRadius:999, background:'var(--pg-ink-100)', color:'var(--pg-ink-700)'}}>🔑 {lang==='pt'?'Portão':'Gate'}</span>}
                    {p.doorman && <span style={{fontSize:11, fontWeight:700, padding:'3px 9px', borderRadius:999, background:'var(--pg-ink-100)', color:'var(--pg-ink-700)'}}>🧑‍💼 {lang==='pt'?'Portaria':'Doorman'}</span>}
                  </div>
                  {p.notes && (
                    <p style={{margin:'8px 0 0', fontSize:12, lineHeight:1.5, color:'var(--pg-ink-600)'}}>{p.notes}</p>
                  )}
                </div>
              ))}
              {!canSeeAddress && (
                <div style={{fontSize:11.5, color:'var(--pg-ink-400)', display:'flex', alignItems:'center', gap:6}}>
                  {Icon.lock ? Icon.lock(11,'var(--pg-ink-400)') : null}
                  {lang==='pt'?'Endereços aparecem depois que sua candidatura for aceita.':lang==='es'?'Las direcciones aparecen después de que tu solicitud sea aceptada.':'Addresses appear once your application is accepted.'}
                </div>
              )}
            </div>
          );
