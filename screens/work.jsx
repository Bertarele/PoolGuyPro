...
function WorkScreen({ ctx }) {
  const { lang, user: ctxUser, openChat, goTab, openPostMenu, openApplicants, openJobDetail,
          ...
          liveApplications=[], jobApplicantCounts={}, refreshLiveApplications,
  ...
  React.useEffect(() => { try { localStorage.setItem('pg_work_sub', sub); } catch(e) {} }, [sub]);
  ...
  const FabBtn = (
    <button onClick={handlePostBtn} className="pg-press" style={{
      position:'fixed', bottom:'calc(96px + env(safe-area-inset-bottom, 0px))', right:18, zIndex:35,
  ...
  const vacCompleteOverlay = (
    <Sheet open={!!completingVacApp} onClose={()=>setCompletingVacApp(null)} height="auto">
      {completingVacApp && (
        <VacCompletionSheet
          app={completingVacApp} lang={lang} showToast={showToast}
          applicantName={ctxUser?.name}
          onClose={()=>setCompletingVacApp(null)}
          onSubmitted={()=>{ setCompletingVacApp(null); refreshLiveApplications && refreshLiveApplications(); }}
        />
      )}
    </Sheet>
  );
    ...
    .map(a => {
      ...
      return {
        ...
        status:       a.status === 'completed' ? 'completed'
                    : a.status === 'accepted' && a.pool_guy_done ? 'done_pending'
  ...
  const myLiveApps = liveApplications
    .filter(a => !liveVacations.some(v => v._id === a.job_id))
    .map(a => {
  ...
  const deleteApp = React.useCallback(async (app) => {
    if (app._live && window.sb) {
      ...
      // pool guy vanished.
      const { data: rows, error } = await window.sb.from('job_applications')
        .delete().eq('id', app.id).neq('status', 'accepted').select('id');
      if (error) { showToast && showToast('❌ ' + error.message); return; }
            ...
            <>
              <div style={{background:_bg, padding:'18px 36px 52px', position:'relative', overflow:'visible'}}>
                ...
                <div style={{position:'absolute', inset:0, overflow:'hidden', pointerEvents:'none', zIndex:0}}>
                  <img src="icone-watermark.webp" alt="" style={{position:'absolute', left:'50%', top:'50%', transform:'translate(-50%,-50%)', height:245, objectFit:'contain', opacity:0.60, userSelect:'none'}}/>
                </div>
                      ...
                      {visibleApps.map((app, i) => {
                        ...
                        const sCfg = isAwaiting
                          ...
                          : isDone
                            ? {label:lang==='pt'?'Concluída ✓':lang==='es'?'Completado ✓':'Completed ✓',color:'oklch(0.40 0.18 145)',bg:'oklch(0.95 0.05 145)'}
                            : isDonePending
                              ? {label:lang==='pt'?'Aguardando confirmação':lang==='es'?'Esperando confirmación':'Awaiting confirmation',color:'oklch(0.48 0.16 260)',bg:'oklch(0.95 0.04 260)'}
                            : isAcc
            ...
            title={
              <div style={{display:'flex', alignItems:'center', gap:10}}>
                ...
                <div>
                  ...
                  <div style={{fontFamily:'var(--pg-font-display)', fontSize:sub==='hiring'?17:20, fontWeight:700, letterSpacing:'-0.025em', lineHeight:1.1, color:H.text, whiteSpace:'nowrap'}}>
                    {sub==='hiring'
                      ? (lang==='pt'?'Vagas de Emprego':lang==='es'?'Ofertas de Trabajo':'Job Openings')
                      : sub==='techs'
                        ? (lang==='pt'?'Técnicos Disponíveis':lang==='es'?'Técnicos Disponibles':'Available Techs')
            ...
            <div style={{display:'flex', alignItems:'center', gap:14, marginTop:10, paddingTop:10, borderTop:`1px solid ${H.border}`}}>
              {sub === 'hiring' && <>
                ...
                <button onClick={()=>setWorkLocationFilterOpen(true)}
                  style={{display:'flex', alignItems:'center', gap:6, background:workUserLocation?'var(--pg-aqua-100)':'rgba(0,178,169,0.10)', border:'1.5px solid var(--pg-aqua-400)',
                    borderRadius:999, padding:workUserLocation?'7px 14px':'7px 12px', boxShadow:'none', cursor:'pointer', fontFamily:'inherit', color:'inherit', touchAction:'manipulation'}}>
                  <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke={workUserLocation?'var(--pg-aqua-600)':'var(--pg-aqua-500)'} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 21s7-6.5 7-12a7 7 0 1 0-14 0c0 5.5 7 12 7 12Z" fill={workUserLocation?'var(--pg-aqua-400)':'none'}/><circle cx="12" cy="9" r="2.5" fill={workUserLocation?'white':'none'}/></svg>
                  {workUserLocation && <span style={{fontSize:12,fontWeight:600,color:'var(--pg-aqua-700)',whiteSpace:'nowrap'}}>{workLocCity ? `${workLocCity} · ` : ''}{workRadiusMiles} mi</span>}
                </button>
              </>}
...
function HandoffDetailPanel({ handoff, user, lang, showToast, onClose, onChat, onEdit, onChanged, openPublicProfile }) {
  ...
  const [localPools, setLocalPools] = React.useState(() => handoff.pools ? handoff.pools.map(p => ({...p})) : null);
  ...
  const togglePoolFilled = async (i) => {
    ...
    const updated = localPools.map((p,j) => j===i ? { ...p, filled: !p.filled } : p);
    const allFilled = updated.every(p => p.filled);
    setBusy(true);
    const { error } = await window.sb.from('pool_handoffs')
      .update({ pools: updated, status: allFilled ? 'filled' : 'open' })
      .eq('id', handoff._id);
    setBusy(false);
    if (error) { showToast && showToast('❌ ' + (error.message||'Error')); return; }
    setLocalPools(updated);
    showToast && showToast(updated[i].filled
      ? (lang==='pt'?'✓ Piscina marcada como preenchida':lang==='es'?'✓ Piscina marcada como cubierta':'✓ Pool marked as filled')
      : (lang==='pt'?'Piscina reaberta':lang==='es'?'Piscina reabierta':'Pool reopened'));
    onChanged && onChanged();
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', height:'100%'}}>
      ...
      <div style={{flex:1, overflow:'auto', padding:'0 18px', display:'flex', flexDirection:'column', gap:14}}>
        ...
        {localPools && localPools.length > 0 ? (
          <div style={{display:'flex', flexDirection:'column', gap:10}}>
            {localPools.map((p, i) => (
              <div key={i} style={{borderRadius:14, border:'1px solid var(--pg-ink-200)', padding:'12px 14px', opacity: p.filled ? 0.55 : 1}}>
                <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8, gap:8}}>
                  <div style={{fontSize:14, fontWeight:800, color:'var(--pg-ink-900)', flex:1, minWidth:0}}>
                ...
                {canManage && (
                  <button onClick={()=>togglePoolFilled(i)} disabled={busy} style={{
                    marginTop:10, width:'100%', height:36, borderRadius:10, cursor:'pointer', fontFamily:'inherit', fontSize:12.5, fontWeight:700,
                    ...
                    {p.filled ? (
                      <>{Icon.check ? Icon.check(13,'currentColor') : null} {lang==='pt'?'Preenchida — reabrir':lang==='es'?'Cubierta — reabrir':'Filled — reopen'}</>
                    ) : (
                      lang==='pt'?'Marcar esta piscina como preenchida':lang==='es'?'Marcar esta piscina como cubierta':'Mark this pool as filled'
                    )}
                  </button>
                )}
                {!canManage && p.filled && (
                  <div style={{marginTop:8, fontSize:11.5, fontWeight:700, color:'var(--pg-ink-400)'}}>
                    {lang==='pt'?'✓ Já preenchida':lang==='es'?'✓ Ya cubierta':'✓ Already filled'}
                  </div>
                )}
              </div>
            ))}
      ...
      <div style={{padding:'12px 18px', flexShrink:0, background:'var(--pg-white)', borderTop:'0.5px solid var(--pg-ink-200)'}}>
        {isOwn ? (
          ...
          !localPools && handoff.status === 'open' && (
      ...
      {[...filteredLiveTechsList].sort((a, b) => {
        ...
        <article key={tech._id} className="pg-card" style={{padding:'14px 16px', position:'relative'}}>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:6, marginBottom:10, fontSize:12.5, color:'var(--pg-ink-500)'}}>
            <div style={{display:'flex', alignItems:'center', gap:12, flexWrap:'wrap'}}>
              {tech.loc && <span style={{display:'inline-flex', alignItems:'center', gap:5}}>{Icon.pin(13,'var(--pg-ink-500)')} {tech.loc}</span>}
              {tech.specialty && <span style={{display:'inline-flex', alignItems:'center', gap:5}}>{Tool(13,'var(--pg-ink-500)')} {tech.specialty}</span>}
            </div>
            <div style={{display:'inline-flex', alignItems:'center', gap:5}}>
          ...
          <div style={{display:'flex', alignItems:'center', justifyContent:'space-between'}}>
            <div style={{fontFamily:'var(--pg-font-display)', fontSize:16, fontWeight:700,
              color: rateIsFixed ? 'var(--pg-blue-500)' : 'var(--pg-aqua-700)', letterSpacing:'-0.01em'}}>
              {rateDisplay}
            </div>
            <div style={{display:'flex', gap:8}}>
              ...
              <button onClick={()=>setContactOpen(isOpen ? null : tech._id)}
                ...
                {isOpen ? (lang==='pt'?'Fechar':lang==='es'?'Cerrar':'Close') : t.contact}
              </button>
            </div>
          </div>
            ...
            <div style={{marginTop:10, padding:'12px 14px', borderRadius:12,
              ...
              {tech.phone && (
                <a href={`tel:${tech.phone}`} style={{display:'flex', alignItems:'center', gap:12, textDecoration:'none'}}>
                  ...
                  <div>
                    <div style={{fontSize:10.5, fontWeight:700, letterSpacing:'0.06em', color:'var(--pg-ink-400)'}}>{lang==='pt'?'TELEFONE':lang==='es'?'TELÉFONO':'PHONE'}</div>
            ...
            {sortedLiveVac.map(vac => {
              ...
              <article key={vac._id} ref={el => { vacCardRefs.current[vac._id] = el; }} className="pg-card" style={{padding:0, overflow:'hidden',
                ...
                <div style={{
                  ...
                  <div style={{textAlign:'right', paddingRight:26, flexShrink:0}}>
                    {vac.priceMode !== 'neg' ? (<>
                ...
                <div style={{padding:'11px 14px 14px'}}>
                  ...
                  {/* Applicants banner — its own full-width row instead of
                      squeezed into the Editar/Encerrar button row, which broke
                      onto a second, overlapping line whenever there actually
                    ...
                    <button onClick={()=>onViewApplicants && onViewApplicants({ ...vac, type:'vacation',
                      ...
                      <div style={{flex:1, minWidth:0, textAlign:'left'}}>
                        ...
                        <div style={{fontSize:11, opacity:0.8}}>
                          {lang==='pt'?'Toque para ver, conversar e aprovar':lang==='es'?'Toca para ver, chatear y aprobar':'Tap to view, message and approve'}
                        </div>
                      </div>
                      {Icon.chev(15,'rgba(255,255,255,0.8)')}
                    </button>
                  )}
...
function VacCompletionSheet({ app, lang='en', onClose, onSubmitted, showToast, applicantName }) {
  ...
  const submit = async () => {
    ...
    if (app.author_id) {
      window.sb.from('notifications').insert({
        ...
        title:   JSON.stringify({ en:'Vacation coverage finished', pt:'Cobertura de férias concluída', es:'Cobertura de vacaciones terminada' }),
        body:    JSON.stringify({
      ...
      window.sendPush && window.sendPush(
        app.author_id,
        lang==='pt' ? '✅ Cobertura concluída' : lang==='es' ? '✅ Cobertura terminada' : '✅ Coverage finished',
        lang==='pt' ? `${applicantName || 'O pool guy'} terminou — confira as fotos e confirme.`
          : lang==='es' ? `${applicantName || 'El pool guy'} terminó — revisa las fotos y confirma.`
          : `${applicantName || 'The pool guy'} finished — review the photos and confirm.`,
        '/#work', 'work'
      );
    }
  ...
  return (
    <div style={{padding:'8px 18px calc(18px + env(safe-area-inset-bottom, 0px))'}}>
      <div style={{fontSize:16, fontWeight:700, textAlign:'center', margin:'6px 0 4px', color:'var(--pg-ink-900)'}}>
        {lang==='pt'?'Finalizar cobertura':lang==='es'?'Finalizar cobertura':'Finish coverage'}
      </div>
      <div style={{fontSize:12.5, color:'var(--pg-ink-500)', textAlign:'center', marginBottom:16, lineHeight:1.4}}>
        {hasTypedRequirement
          ? (lang==='pt' ? 'Tire ou selecione cada foto abaixo antes de finalizar.' : lang==='es' ? 'Toma o selecciona cada foto abajo antes de finalizar.' : 'Take or select each photo below before finishing.')
          : (lang==='pt'
            ? 'Tire pelo menos uma foto das piscinas que você cobriu. O dono vai revisar e confirmar.'
            : lang==='es'
              ? 'Toma al menos una foto de las piscinas que cubriste. El dueño revisará y confirmará.'
              : 'Take at least one photo of the pools you covered. The owner will review and confirm.')}
      </div>
      ...
      <button onClick={submit} disabled={!canSubmit || submitting} className="pg-btn pg-btn-primary"
        style={{width:'100%', height:52, fontSize:15, borderRadius:14, marginTop:18, opacity: !canSubmit||submitting?0.45:1}}>
        {submitting
          ? (lang==='pt'?'Enviando…':lang==='es'?'Enviando…':'Sending…')
          : (lang==='pt'?'Enviar e concluir':lang==='es'?'Enviar y concluir':'Submit and finish')}
      </button>
    </div>
  );
}
      ...
      <div style={{flex:1, overflowY:'auto', padding:'20px 20px 100px'}}>
        ...
        <div style={{display:'flex', gap:24, alignItems:'flex-start', flexWrap:'wrap'}}>
          ...
          <div style={{flex:'1 1 260px', minWidth:240, display:'flex', flexDirection:'column', gap:16}}>
            ...
            {/* Required photos — same picker as Piscinas Rápidas/Rotas Rápidas */}
            {selectedDays.size > 0 && (
              <div style={{marginTop:16, borderRadius:14, border:'1px solid var(--pg-ink-200)', padding:'14px 16px'}}>
                ...
                <div style={{fontSize:11, color:'var(--pg-ink-500)', marginBottom:12}}>
                  {lang==='pt'?'O pool guy deverá tirar essas fotos antes de finalizar cada dia.':lang==='es'?'El pool guy deberá tomar estas fotos antes de finalizar cada día.':'The pool guy must take these photos before finishing each day.'}
                </div>
                <div style={{display:'flex', flexDirection:'column', gap:8}}>
                  {QUICK_POOL_PHOTO_OPTS.map(opt => {
                    const sel = requiredPhotos.includes(opt.key);
                    return (
                      <button key={opt.key} onClick={()=>toggleVacPhoto(opt.key)} style={{
                  ...
                  <div style={{display:'flex', gap:8, marginTop:2}}>
                    <input className="pg-field" value={customPhotoText}
                      ...
                      onKeyDown={e=>{ if(e.key==='Enter'){ e.preventDefault(); addCustomVacPhoto(); } }}/>
                    <button onClick={addCustomVacPhoto} style={{
...
function blankPool() {
  return { city:'', days:[], price:'', poolType:'residential', dog:false, saltwater:false, gateCode:false, doorman:false, photos:[], description:'', filled:false };
}
function poolsFromInitialValues(initialValues) {
  ...
  if (Array.isArray(initialValues.pools) && initialValues.pools.length > 0) {
    return initialValues.pools.map(p => ({
      ...
      description: p.description || '', filled: !!p.filled,
    }));
  }
      ...
      <div style={{padding:'12px 18px', flexShrink:0, background:'var(--pg-white)', borderTop:'0.5px solid var(--pg-ink-200)'}}>
        ...
        <button onClick={()=>onSubmit && onSubmit({
          pools: pools.map(p => ({
            ...
            description: p.description || null, filled: !!p.filled,
          })),
...
function VacationDayPickerSheet({ vac, lang='en', onClose, onSubmit, confirmedDays=[], myAppliedDays=[] }) {
  ...
  return (
    <div style={{padding:'0 18px 0', display:'flex', flexDirection:'column', minHeight:'100%'}}>
      ...
      {/* Required photos preview — so the applicant knows what proof they'll
          need to submit before they commit to days, same as QuickPools shows
          in its job detail before someone applies. */}
      {vac.requiredPhotos && vac.requiredPhotos.length > 0 && (
        <div style={{marginTop:14, padding:'10px 12px', borderRadius:10, background:'var(--pg-aqua-50)', borderLeft:'3px solid var(--pg-aqua-500)'}}>
          <div style={{fontSize:10.5, color:'var(--pg-aqua-700)', fontWeight:700, letterSpacing:'0.05em', textTransform:'uppercase', marginBottom:6}}>
            📸 {lang==='pt'?'Fotos obrigatórias':lang==='es'?'Fotos obligatorias':'Required photos'}
          </div>
          <div style={{display:'flex', flexWrap:'wrap', gap:6}}>
            {vac.requiredPhotos.map((p,i) => {
              const label = p.startsWith('custom:') ? p.slice(7)
                ...
                : p==='chemical' ? (lang==='pt'?'Foto químico':'Chemical photo') : p;
              return (
                <span key={i} style={{fontSize:11, fontWeight:600, color:'var(--pg-ink-700)', background:'var(--pg-white)', padding:'3px 9px', borderRadius:999, border:'0.5px solid var(--pg-aqua-100)'}}>
                  {label}
                </span>
              );
            })}
